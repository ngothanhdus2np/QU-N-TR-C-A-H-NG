...
export const generateId = (): string => {
  // globalThis.crypto hoạt động trong browser (Chrome/Firefox/Safari)
  // và Node.js 17+ (Web Crypto API được expose global từ Node 17).
  // Node 14-16 không có global crypto nhưng có require('crypto').randomUUID —
  // fallback Math.random chỉ xảy ra trên môi trường rất cũ (không còn supported).
  if (typeof globalThis.crypto?.randomUUID === 'function') {
    return globalThis.crypto.randomUUID();
  }
  // Math.random fallback — chỉ cho môi trường Node < 17 (không còn phổ biến)
...
export const processExcelRawData = (data: ArrayBuffer | string, isExcel: boolean): ParsedExcelRow[] => {
