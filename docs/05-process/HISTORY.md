...
- **CHƯA chạy trên prod thật** — chỉ mới local (bản copy prod) + dev riêng biệt. Bảng backup `backfill_discount_backup_0814` còn giữ lại ở cả 2 nơi để rollback nếu cần (chưa dọn).
- Files: không có file code nào bị sửa — chỉ chạy SQL trực tiếp trên 2 Supabase instance (local + dev).
