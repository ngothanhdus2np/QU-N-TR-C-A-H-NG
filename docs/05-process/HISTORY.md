...
- Kiểm tra bắt buộc: `tsc --noEmit` sạch, `npm test` 448/448 pass, kill+restart dev server nhiều lần trong phiên, verify cả 3 fix bằng giao dịch thật trên browser. Cập nhật `docs/business-knowledge/FORMULAS.md` §8.7 (công thức `customerDebt`, ghi rõ quy tắc loại trừ tránh đếm trùng). Dọn sạch toàn bộ khách hàng/sản phẩm/đơn hàng test khỏi Supabase local sau khi xong.
