...
- **`908706b` — gộp Code128 (3 bản → 1)**: thuật toán tồn tại y hệt ở `barcodeUtils.ts`, `GoodsInventory.tsx`, `PrintTemplatesTab.tsx`. Đối chiếu **106 pattern của cả 3 bảng → giống hệt nhau**; chỉ khác `height` (40 vs 50) và có/không `aria-label`. Thay vì ép về một kiểu, đưa 2 khác biệt đó thành tham số nên mỗi nơi gọi giữ nguyên đầu ra. **Height không phải chuyện thẩm mỹ**: CSS tem dùng `height: auto` nên số trong viewBox chính là tỉ lệ khung hình — đổi là đổi độ cao mã vạch in ra giấy. **Cố ý KHÔNG gộp** phần HTML/CSS của tem (thẻ `<div>` vs `<section>`, hậu tố `đ` vs ` VNĐ`, tên SP lấy khác nhau) vì gộp sẽ đổi diện mạo tem thật. Verify: trích bản gốc từ git rồi so đầu ra trên 8 input (có dấu tiếng Việt, chuỗi rỗng, toàn khoảng trắng, quá dài, ký tự đặc biệt) — **trùng khít 8/8**. Thêm 6 test. Net −144 dòng.
- **`fae13b8` — gộp helper `components/orders/`**: `fmt`, `PAYMENT_LABELS`, `PAYMENT_METHODS`, `customerCodeMap`, `getCustomerCode` bị copy giữa 3 trang. Đây là gộp có bug đứng sau: chính các bản sao đã lệch (`OrderReturns` thiếu khoá `Card`, đã vá ở `30f7634`). **Cố ý KHÔNG gộp** `getCustomerCode` của `PendingOrdersPage` — nó cắt UUID, khác hẳn bản tra bảng KH000001, trùng tên nhưng khác hành vi. Thêm 7 test, trong đó 1 test khẳng định MỌI `PAYMENT_METHOD` phải có nhãn — đúng bất biến từng bị phá. Kiểm chứng: bỏ lại khoá `Card` thì test fail với thông báo "thiếu nhãn cho Card". Net −65 dòng.
- **`b658d7b` — gộp `parseSku`/`BUCKET` ở `revenue/`** (2 bản đã lệch: bản ở `SourceDetailPage` thiếu `parentCode`) + **8 test HTTP cho `factoryReset.ts`**. Endpoint này xoá vĩnh viễn toàn bộ dữ liệu + tài khoản, không hoàn tác, không backup — và **chưa từng có test nào**, dù từng dính lỗ hổng phân quyền (FACTORY-RESET-SECURITY-0816). Các test chặn đều khẳng định **không bảng nào, không tài khoản nào bị đụng**, chứ không chỉ kiểm mã 403. Kiểm chứng: tạm gỡ `requireOwner` → đúng 3 test chặn fail.
- **`a891f02` — 13 test cho `posOfflineQueue.ts`**: 308 dòng, không test, trong khi bản vá `mergeBy` (SYNC-FORCE-RESURRECT-0819) dựa hẳn vào nó — hàng đợi sai nghĩa là dữ liệu nhân viên nhập lúc mất mạng biến mất. Test phủ cả 4 chuyển trạng thái gộp thao tác + các trường hợp KHÔNG được gộp (khác id, cùng id khác bảng, `pushBatch`, payload không có id). Kiểm chứng: phá nhánh upsert→delete thì đúng test đó fail. Thêm `fake-indexeddb` (devDependency, 1 gói, 0 phụ thuộc, 0 lỗ hổng).
- **`b4e8fae` — bật `strictFunctionTypes`**: đo trước tất cả các cờ (`noImplicitAny` 31, `noImplicitThis` 58, `strictNullChecks` 111, `strict` đủ 162) rồi chọn cờ 4 lỗi để làm dứt điểm. Sửa 5 chênh lệch, trong đó **1 cái là bẫy thật**: `ChatInterface`/`FloatingCFOChat` khai `setMessages` kiểu `Dispatch<SetStateAction>` (quảng cáo dùng được updater), nhưng hàm truyền vào thật là `setChatMessages` — dispatcher reducer chỉ nhận MẢNG. Viết `setMessages(prev => ...)` theo thói quen React sẽ qua được biên dịch rồi lưu thẳng cái HÀM vào chỗ mảng tin nhắn. Đã thu hẹp kiểu để lỗi đó bị chặn lúc biên dịch. Khôi phục generic cho `pushBatch` ở 6 nơi (bản cũ `unknown[]` khiến `pushBatch('posOrders', [nhânViên])` vẫn compile được) — và generic vừa khôi phục **lập tức bắt được** ngoại lệ [IMPORT-02] có chủ đích ở `useGoodsExcelImport`, nay đã khai rõ bằng cast kèm giải thích thay vì bị `unknown[]` che.
- **Tổng Giai đoạn 5**: net khoảng −200 dòng trùng lặp, **test 452 → 486** (+34).
- Sau mỗi commit: `tsc` sạch, `npm run lint` exit 0, `npm test` pass, `npm run build` exit 0. Verify browser: `/goods`, `/order-invoices`, `/order-returns`, `/report-sales`, `/settings` đều render đủ, không lỗi JS.
- **CHƯA deploy** — iMac vẫn offline.
...
- **`6762d82` (2a — backend + trang report)**:
  - `server.ts`: gỡ đăng ký `/health` GET+HEAD trùng lần 2 bên trong `startServer()` (Express khớp theo thứ tự, handler ở module scope phía trên đã trả response và không gọi `next()` nên cặp này không bao giờ chạy). **Verify sau khi gỡ**: `GET`/`HEAD /health` vẫn vào đúng handler thật (503 `DB_UNAVAILABLE` từ bước check DB), trong khi path lạ rơi vào SPA fallback trả 200 → chứng minh route còn nguyên.
  - `server.ts`: gỡ middleware log riêng cho `/api` — logger `[DEV Request]` phía trên đã log MỌI request kèm timestamp nên `/api/*` bị log 2 lần.
  - `server.ts`: gỡ `import pg from 'pg'` — **lần xuất hiện duy nhất của `pg` trong cả repo**, không có `pg.Pool`/`pg.Client`/`new Pool` ở đâu (migration dùng `psql` ngoài Node). Gỡ luôn `pg` + `@types/pg` khỏi `package.json`, sau đó **khởi động server thật** xác nhận vẫn boot sạch. Gỡ kèm import thừa `NextFunction`, `readFile`.
  - `services/errorTracking.ts` (186 → 143 dòng): xoá `captureMessage`, `setUser`, `clearUser`, `addBreadcrumb` — 0 caller trong toàn repo, cả 4 chỉ chứa nhánh `if (this.sentryEnabled)` với `sentryEnabled` hard-code `false`, tức no-op thật sự. Field `isProduction` đi theo `addBreadcrumb` (người đọc duy nhất). Giữ nguyên `init()`, `captureError()`, `errorHandler`.
  - `routes/adminStore.ts`: gỡ import `Request`/`Response` và hằng `EMAIL_RE` không dùng.
  - `components/reports/*` (8 trang): xoá `SelectButton` (định nghĩa 7 lần, không dùng cả 7) và `applyWeek` (7 lần, không dùng cả 7) — tàn dư của việc copy nguyên trang trước khi `ReportRangeTimeFilter`/`ReportDropdownFilter` dùng chung thay thế.
- **`2e2f959` (2b — 88 import thừa ở 33 file)**: bám hoàn toàn theo danh sách `no-unused-vars` của eslint (chỉ đáng tin sau khi Giai đoạn 1 sửa cổng lint). **Chỉ đụng import specifier**; cố ý KHÔNG đụng tham số hàm/prop destructure vì đổi chữ ký, và vài cái (vd `onDeleteSupplier` ở `SupplierListPage`) có thể là dấu hiệu tính năng chưa nối dây cần quyết định riêng. Cụm lớn nhất: `SettingsCenter` (15), `KnowledgeManager` (17), `CustomerListPage` (6). Script xử lý đúng cả import có alias — giữ `CalendarCheck as DateIcon`, xoá `CalendarCheck` trần. Vì import là chuyện lúc build nên **chạy `npm run build` thật** chứ không chỉ tin tsc.
- **`30f7634` (2c — bản sao còn sót)**: xoá `AllowanceInput`/`AllowanceRow` (33 dòng widget phụ cấp lương lạc trong `KnowledgeManager.tsx`, không dùng — `InputWrapper` ngay sau đó vẫn dùng nên giữ). Xoá `toggleAllowSellOutOfStock` khỏi `SettingsCenter.tsx` — UI thật nằm ở `settings/tabs/GoodsTab.tsx`, **verify trên browser sau khi xoá**: toggle "Cho phép bán hàng khi hết tồn kho" vẫn hiện đúng ở Thiết lập → Hàng hóa, trạng thái "Đang khóa". Thêm khoá `Card: 'Thẻ'` thiếu ở `OrderReturns.tsx` (2 bản sao kia đều có, bản này lệch) — `Card` là phương thức có thật (`POSCheckout` hỗ trợ) nên đơn trả hàng thanh toán thẻ đang rơi vào fallback hiện chữ Anh "Card". Chỉ sai hiển thị, không mất dữ liệu.
- **`0289667` (gộp `getErrorMessage`)**: hàm 16 dòng bị copy y hệt ở 5 file (`data`/`import`/`facebook`/`notifications`/`factoryReset`); bản thứ 6 ở `ai.ts` khác đúng 1 điểm — fallback `'AI service error'` thay vì `'Unknown error'`. Tạo `routes/errorMessage.ts` nhận fallback làm tham số, `ai.ts` giữ wrapper 1 dòng nên 23 lời gọi cũ không phải sửa. **Đối chiếu hành vi với bản gốc trên 10 input** (Error, object kiểu Supabase, chỉ có `code`, object rỗng, mảng rỗng, `null`, `undefined`, string, số 0, tham chiếu vòng lặp) — **khớp 10/10** cho cả biến thể chung lẫn `ai.ts`. Ghi chú: fallback thực ra chỉ áp dụng cho giá trị KHÔNG phải object — `{}` trả `"{}"` qua `JSON.stringify`, object vòng lặp trả `"[object Object]"` qua nhánh catch; đúng như trước.
- **Verify**: `tsc --noEmit` sạch, `npm run lint` exit 0, `npm test` 448/448, `npm run build` exit 0 — chạy lại sau MỖI commit. Trên browser (dev server local): trang Bán hàng/Tài chính/Kênh bán hàng và Thiết lập render đầy đủ, **không có lỗi JS/module nào** trong console (chỉ còn nhiễu 401/503/CORS sẵn có do máy này không nối được Supabase trên iMac).
- Cảnh báo `no-unused-vars`: **184 → 79**.
- **Chưa deploy dev/prod** — chờ user quyết.
...
- User muốn mã sản phẩm ở "Phiếu nhập hàng" bấm được, mở trang chi tiết sản phẩm (dẫn ảnh chụp màn hình KiotViet làm mẫu). Làm qua nhiều vòng lặp theo phản hồi trực tiếp của user:
  1. **Bấm mã hàng mở tab mới**: `PurchaseOrderInlineDetail.tsx`, `PurchaseOrderDetailModal.tsx` đã có sẵn `navigate('/goods?edit=<id>')` khi bấm SKU (chỉ thiếu ở `GoodsPurchaseForm.tsx` — form thật của "Phiếu nhập hàng", user tự phát hiện qua test thực tế). Đổi cả 3 nơi sang `window.open(url, '_blank')` để mở tab mới thay vì điều hướng cùng tab.
  2. **`/goods?edit=` mở nhầm form sửa thô** (`showCreateModal`), không phải panel chi tiết kiểu KiotViet (`GoodsProductDetailPanel` — vốn đã có sẵn, giống hệt tham chiếu KiotViet user gửi, nhưng chỉ hiện khi bấm trực tiếp 1 dòng trong bảng, không mở được từ URL). **Đã thêm**: query param mới `?view=<id>` (`GoodsInventory.tsx`) — xóa hết filter/search đang áp dụng, tính đúng trang chứa sản phẩm trong danh sách ĐẦY ĐỦ (không lọc riêng theo SKU, theo đúng yêu cầu user — khác với thử nghiệm đầu tiên là lọc còn 1 dòng), tự mở rộng dòng cha nếu là biến thể, mở `GoodsProductDetailPanel` ngay dòng đó. Đổi cả 3 nơi bấm mã hàng sang `/goods?view=` thay vì `/goods?edit=`.
  3. **Tự cuộn tới panel** (user không muốn tự vuốt xuống): thêm `data-viewing-product-panel` + effect tự `scrollIntoView`. Phát hiện bug lúc verify: `behavior: 'smooth'` không chạy trong container bảng này (im lặng không làm gì, xác nhận qua debug log) — đổi sang `behavior: 'instant'` thì đúng.
  4. **Bug nghiêm trọng riêng do user phát hiện**: dữ liệu "Phiếu nhập hàng"/"Trả hàng nhập" đang nhập dở bị mất hoàn toàn nếu chuyển sang tab khác (state cục bộ trong `usePurchaseFormState.ts`, unmount là mất, không có tự lưu). **Đã sửa**: tự lưu nháp vào `localStorage` (`cfo_brain_purchase_draft_v1`, `cfo_brain_purchase_return_draft_v1`, loại trừ `invoiceFile` vì File không serialize được) mỗi khi dữ liệu đổi; khôi phục lại state khi component mount lại. Theo yêu cầu tinh chỉnh của user: **không** tự nhảy vào form khi quay lại (mặc định luôn hiện danh sách phiếu) — chỉ khôi phục ngầm vào state, và hiện cảnh báo "Tiếp tục nhập / Tạo phiếu mới" khi user chủ động bấm "+ Phiếu nhập hàng" (áp dụng tương tự cho "+ Trả hàng nhập" và "Mở phiếu" một đơn khác). Ban đầu dùng `window.confirm` (popup Chrome) — user phản hồi không đúng UI app → đổi sang component có sẵn `ConfirmDialog.tsx` (đã dùng ở `GoodsInventory.tsx`...).
...
- User báo logo cửa hàng ở Cài đặt → Cửa hàng bị vỡ ảnh. Điều tra qua 3 lớp lỗi chồng nhau:
  1. **Link chết**: `brand_profile.logo` (DB dev) trỏ project Supabase Cloud CŨ `tqouzxlnihfjdyxqlbqs.supabase.co` (đã ngừng dùng từ khi hệ thống chuyển sang self-host, domain thật hiện tại là `supabase.phucsang.com.vn`/`supabase-dev.phucsang.com.vn` — xác nhận qua CSP `server.ts:426`). Domain cũ không resolve được nữa (`curl`: Could not resolve host). **Đã sửa**: `UPDATE brand_profile SET logo = NULL WHERE id='00000000-0000-0000-0000-000000000000'` trên `supabase-db-dev` (qua SSH iMac). Client cache `localStorage['cfo_brain_brand_profile']` phải xóa tay + reload mới hết hiện ảnh vỡ (cache cũ không tự invalidate theo giá trị field mới, chỉ theo còn/mất record).
  2. **Thiếu RLS**: user upload lại logo mới → toast "Lỗi tải ảnh lên Cloud!". Tra `pg_policies` xác nhận `storage.objects` bật RLS nhưng **0 policy nào cho toàn schema storage** trên dev (bucket `images` tồn tại, public=true, nhưng INSERT/UPDATE/DELETE qua client luôn cần RLS bất kể bucket public). Bucket `knowledge-files` có policy sẵn (dùng cho PDF Quy chuẩn) nên không bị ảnh hưởng, còn `images` (dùng cho logo + Marketing AI) thì chưa từng được cấp policy khi hạ tầng chuyển sang self-host. **Đã sửa**: thêm 4 `CREATE POLICY` cho bucket `images` trên `supabase-db-dev` (`images public read` SELECT, `images authenticated upload/update/delete` — chỉ role `authenticated`, không cho `anon`, đúng pattern bảo mật đã áp dụng cho `knowledge-files`). Verify qua fetch trực tiếp trong browser (dùng session JWT thật của user đang đăng nhập, đúng flow client-side supabase-js) — request vượt qua được RLS, lỗi đổi sang lớp khác.
  3. **Lỗi hạ tầng Docker (CHƯA đóng — cần user ở gần iMac)**: lỗi mới là `500 The file system does not support extended attributes or has the feature disabled`. Xác nhận qua tài liệu chính thức Supabase + issue GitHub `supabase/supabase#3902`/`#10745`: container `supabase-storage` dùng bind mount (`STORAGE_BACKEND=file`) từ thư mục thật trên iMac — bind mount trên Docker Desktop macOS không hỗ trợ xattr (đã tự test: xattr hoạt động bình thường ngay trên host APFS, chỉ lỗi khi đi qua lớp chia sẻ file của Docker vào container). **Ảnh hưởng CẢ dev lẫn prod** (`docker inspect` xác nhận `supabase-storage` prod và `supabase-storage-dev` cùng cấu hình bind-mount, cùng version `v1.60.4`; cả 2 thư mục storage đang có 0 file — upload qua Storage client-side chưa từng thành công từ trước tới giờ, không phải mới hỏng, không rủi ro mất dữ liệu khi sửa). Hướng sửa chính thức: cả 2 môi trường đã có sẵn override `docker-compose.rustfs.yml` (đổi sang S3-compatible RustFS, named volume, tránh hẳn lỗi xattr) + script quản lý `run.sh config add/remove` + `run.sh recreate`. Thử chạy `sh run.sh recreate rustfs rustfs-createbucket storage` trên dev → lỗi pull image `rustfs/rustfs`/`rustfs/rc` vì Docker Desktop cần unlock Keychain, phiên SSH không tương tác nên không unlock được. Đã dọn dẹp: gỡ lại override rustfs khỏi `COMPOSE_FILE` (.env dev) để tránh treo nửa chừng, container storage dev vẫn chạy bình thường như trước (verify qua `docker compose ps`).
- User xác nhận không ở gần iMac → ghi task `STORAGE-XATTR-0821` vào TODO.md (P0) kèm đầy đủ lệnh cần chạy khi có người pull image trực tiếp trên iMac.
- Files: không đổi code app. Đổi dữ liệu (`brand_profile.logo` trên `supabase-db-dev`) + RLS policy (`storage.objects` trên `supabase-db-dev`, bucket `images`).
...
- **Đã sửa (Hướng B — chuyển tính toán sang server)**: thêm route `GET /api/data/customer-stats` (`routes/data.ts`, `requireAuth`, service-role) — phân trang `pos_orders` (slim columns) + `customer_debt_history` ở server, tổng hợp per-customer (sold/returned/debt/lastTransactionDate/spentInRange theo `date_from`/`date_to` optional) bằng đúng công thức JS cũ (không dịch sang SQL để tránh rủi ro sai công thức tài chính). `CustomerListPage.tsx` giờ chỉ gọi route này, không còn fetch toàn bộ đơn hàng; chi tiết 1 khách (`CustomerDetailPage`) chuyển sang fetch riêng qua `apiService.fetchOrdersForCustomer(customerId)` khi mở dòng chi tiết, thay vì dùng chung `allOrders` toàn shop. Xoá `fetchAllOrdersForCustomerStats` (dead code sau khi thay thế).
- **Đã sửa (Hướng A — cache)**: `apiService.fetchCustomerStats()` cache kết quả 60s (module-level, key theo date range) — tránh gọi lại API mỗi lần chuyển tab qua lại trang Khách hàng.
- Thêm index `idx_pos_orders_customer_id` (migration `042_customer_stats_indexes.sql`). Bỏ index cho `customer_debt_history` — role migration trên DB staging không phải owner bảng đó (`must be owner of table customer_debt_history`), và bảng này vốn nhỏ/giới hạn `.limit(5000)` nên không phải nút thắt.
- Cập nhật `docs/business-knowledge/FORMULAS.md` mục 8.6-8.9 (Source đổi từ `CustomerListPage.tsx` sang `routes/data.ts` route mới, công thức toán học giữ nguyên).
- `npm test` 448/448 sạch, `tsc --noEmit` sạch. Verify thật trên dev (`dev.phucsang.com.vn/customers`): `GET /api/data/customer-stats` trả 200, trang hiện đúng 251 khách hàng + tổng nợ/bán/trừ trả hàng hợp lý; mở chi tiết 1 khách xác nhận `fetchOrdersForCustomer` trả đúng đơn hàng, giá trị khớp với "Tổng bán" ở danh sách.
...
- Đã deploy dev. **Chưa đụng prod** — chờ user yêu cầu riêng.
- **[Follow-up cùng ngày] User yêu cầu test lại để xác nhận đã giải quyết chưa** — đo bằng Performance API thật trên `dev.phucsang.com.vn`: lần đo đầu tiên `GET /api/data/customer-stats` mất **9264ms** — chậm bất ngờ. Điều tra: DB dev có **70.528 đơn hàng** (`pos_orders`), route mới phân trang **tuần tự** (`while` loop, await từng trang 2000 dòng) → ~36 round-trip nối tiếp tới Supabase, cộng dồn độ trễ mạng thành gần 9s (log iMac cũng cho thấy `[health] DB check failed: DB health check timeout` lặp lại — DB dev có phần chậm sẵn, nhưng round-trip tuần tự là nguyên nhân chính bị khuếch đại).
  - **Đã sửa**: đổi sang lấy `count: 'exact'` ở trang đầu rồi bắn toàn bộ các trang còn lại bằng `Promise.all` (song song) thay vì `await` tuần tự từng trang — áp dụng cho cả `pos_orders` và `customer_debt_history`. Độ trễ giờ chỉ còn ~1 round-trip (giới hạn bởi trang chậm nhất) thay vì tổng cộng dồn.
  - **Kết quả đo lại**: 825ms rồi 1519ms (2 lần đo liên tiếp) — nhanh hơn 6-11 lần so với 9264ms ban đầu. Số liệu hiển thị (251 khách hàng, nợ/bán/trừ trả hàng) giữ nguyên chính xác như trước khi sửa — xác nhận song song hoá không đổi kết quả tính toán, chỉ đổi cách gọi.
  - `tsc --noEmit` sạch, `npm test` 448/448 pass lại sau fix.
  - Files thêm: `routes/data.ts` (khối `GET /api/data/customer-stats`, phần fetch `pos_orders`/`customer_debt_history`).
- **[Follow-up thứ 2, user báo "cả trang Hàng hoá và Khách hàng đều lỗi" trên dev]** Điều tra bằng curl trực tiếp vào PostgREST (qua SSH iMac) phát hiện: `PGRST_DB_MAX_ROWS=1000` — server **cap cứng mỗi request tối đa 1000 dòng bất kể `.range()` yêu cầu rộng bao nhiêu** (xác nhận thực tế: `range(0,4999)` chỉ trả về 1000 dòng, `Content-Range: 0-999/*`). Code song song hoá ở follow-up trước giả định mỗi trang trả đúng `PAGE=5000` dòng để tính `orderPageCount = ceil(count/PAGE)` — sai giả định này khiến mỗi "trang" giả định 5000 dòng thực ra chỉ nhận 1000 dòng đầu, bỏ sót 80% dữ liệu trong mỗi cửa sổ — **may mắn chưa lộ ra ở DB dev** vì số đơn có gắn `customer_id` thực tế chỉ 267 dòng (nằm gọn 1 trang, dưới ngưỡng 1000), nhưng sẽ âm thầm tính sai khi số đơn tăng lên.
  - **Đã sửa triệt để**: thêm hàm dùng chung `fetchAllPagedRows()` (đầu `routes/data.ts`) — trang đầu tiên tự đo kích thước trang THỰC TẾ server trả về (không hardcode số cụ thể), các trang sau dùng đúng con số đó để không bao giờ bỏ sót dòng dù `PGRST_DB_MAX_ROWS` là bao nhiêu hay đổi sau này. Áp dụng cho cả `pos_orders` và `customer_debt_history`.
  - Không tái hiện được lỗi "cả 2 trang báo lỗi" sau khi test lại trên tab trình duyệt sạch (không phải tab đã dùng để đo timing nhiều lần trước đó — nghi ngờ tab cũ bị lẫn state/cache từ việc điều hướng qua lại liên tục lúc đo). Đã kiểm tra log server trên iMac: có `[health] DB check failed: DB health check timeout` lặp lại nhiều lần — nhưng đối chiếu với network requests thực tế lúc test thì mọi request `/api/*` đều 200 OK, nên health-check-log này không phải nguyên nhân trực tiếp gây lỗi UI (có thể là nhiễu từ giai đoạn app vừa restart, hoặc 1 health-check job riêng với timeout khắt khe hơn — ghi nhận, chưa cần xử lý thêm vì không ảnh hưởng chức năng thực tế đã verify).
  - `tsc --noEmit` sạch, `npm test` 448/448 pass. Verify lại cả 2 trang (Hàng hoá + Khách hàng) trên tab mới, nhiều lần liên tiếp — không lỗi, số liệu đúng.
  - Files thêm: `routes/data.ts` (hàm `fetchAllPagedRows`, refactor khối `customer-stats`).
...
- **Đã deploy PROD** (`app.phucsang.com.vn`/`cfobrain.phucsang.com.vn`) — user xác nhận qua AskUserQuestion trước khi chạy. `scripts/deploy-imac.sh`: backup hardlink bản cũ (rollback tự động nếu health-check fail) → rsync code → `apply-migrations.sh --prod` (migration `037` chạy thành công, ghi sổ `schema_migrations`) → build → restart `com.cfobrain.app` → health-check polling (script prod đã có sẵn cơ chế poll 15×1s, không bị lỗi "exit 7 curl sớm" như bản dev) → `✅ Deploy thành công`. Verify thêm: cột `email` đã có trên `supabase-db` (prod thật), log `/tmp/cfobrain-app.log` không có lỗi khởi động.
- Files: `routes/data.ts`, `components/StaffManager.tsx`, `supabase_setup.sql`, `supabase_migrations/037_employees_email_column.sql` (mới) — đã lên local + dev + prod.
...
- **Điều tra 13 đơn lỗi sau ngày fix (30/06–09/07/2026)** — nguyên nhân: **deploy trễ**, không phải lỗi logic mới. Check file `routes/import.ts` thực tế chạy trên prod (`~/cfobrain/routes/import.ts` trên iMac) — có đúng fix (`discount: Math.max(0, p.totalGross - rev)` dòng 884) nhưng mtime **22/07/2026**, tức fix merge vào git từ 29/06 nhưng chỉ thực sự lên prod ngày 22/07 — trễ ~3-4 tuần. 13 đơn lỗi đều nằm trong đúng khoảng 30/06→09/07 (code cũ vẫn chạy trên server), không có đơn lỗi nào sau 09/07 — khớp hoàn toàn giả thuyết deploy trễ, đã có trong 1.624 đơn backfill hôm nay (query quét theo điều kiện dữ liệu, không giới hạn ngày nên đã bắt hết). Rút kinh nghiệm quy trình: fix liên quan tiền/kho nên deploy sớm sau khi merge.
