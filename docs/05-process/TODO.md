...
> **Bước 2 — XONG 2026-09-17**: 7 màu + font + cỡ chữ + bo góc + mật độ + chuyển động đều chỉnh tay được, ghi ra `--ux-*` trên `<html>` và áp ra toàn app qua khối `html[data-custom="1"][data-theme]` cuối `index.css`. Lưu riêng từng theme trong `localStorage`. 8 test mới, tổng 494. Chi tiết HISTORY.md 2026-09-17 (2).
> **Bước 2 — mô tả gốc (đã hoàn thành)**: biến bảng token trong panel trái thành điều khiển chỉnh tay (màu, bo góc, mật độ, cỡ chữ, chuyển động), ghi thẳng lên `documentElement.style` + lưu `localStorage`, và mở rộng bảng map class Tailwind → biến CSS trong `index.css`. Trường `tokens` trong `constants/themes.ts` đã dựng sẵn cho việc này.
> **Deploy dev XONG 2026-09-18**. Nguyên nhân kẹt trước đó không phải Remote Login mà là **iMac đổi subnet** `192.168.1.x` → `192.168.88.x`; IP cứng trong 6 script đã thay bằng hostname `iMac-cua-mac.local`. Chi tiết HISTORY.md 2026-09-18. **Chưa deploy prod** — cần user yêu cầu riêng.
...
> **Bug gốc**: code có sẵn comment `eslint-disable react-hooks/...` nhưng gói plugin chưa từng được cài → 6 error "rule not found" khiến `npm run lint` luôn fail, và **245 file React chưa bao giờ được kiểm tra dependency array / rules-of-hooks**.
> **Đã sửa**: cài `eslint-plugin-react-hooks@7.1.1` (bản duy nhất hỗ trợ ESLint 10), bật `rules-of-hooks: 'error'` + `exhaustive-deps: 'warn'`. Cố ý KHÔNG bật ~28 rule React Compiler khác của plugin v7. Sửa kèm 2 lỗi `prefer-const`.
> **Kết quả**: `rules-of-hooks` 0 vi phạm. `npm run lint` **exit 0** (từ 8 error), `npm test` 448/448, `tsc` sạch.
> Files: `eslint.config.js`, `package.json`, `package-lock.json`, `components/pos/printInvoiceFromTemplate.ts:346`, `routes/channelLinks.ts:328`.

### [ ] 🟡 HOOKS-DEPS-0829 — 89 cảnh báo `react-hooks/exhaustive-deps` lần đầu lộ ra, cần rà từng cái

> Sau khi cài plugin ở `LINT-GATE-0829`, eslint phát hiện **89 cảnh báo `exhaustive-deps` ở 36 file** — đây là code chưa từng được luật nào soi từ trước tới giờ.
> **Cố ý chưa sửa**: sửa dependency array có thể đổi hành vi runtime (effect chạy thêm/bớt lần, vòng lặp vô hạn, state cũ) — phải rà từng cái kèm verify thật, không được sửa hàng loạt.
> Tập trung nhất: `components/pos/GoodsInventory.tsx` (15), `components/pos/POSComputer.tsx` (8), `hooks/usePayrollState.ts` (8), rồi 33 file còn lại 1-3 cảnh báo mỗi file.
> Lệnh xem danh sách: `npx eslint . -f json` rồi lọc `ruleId === 'react-hooks/exhaustive-deps'`.
> **Gợi ý cách làm**: chia nhỏ theo file, mỗi file 1 commit, verify thật luồng liên quan trên dev trước khi sang file tiếp. Ưu tiên `POSComputer.tsx` (luồng bán hàng) và `usePayrollState.ts` (luồng lương) vì rủi ro nghiệp vụ cao nhất.

### [ ] 🟠 CODEBASE-CLEANUP-0829 — Dọn dead code / trùng lặp theo lộ trình 5 giai đoạn (audit 2026-08-29)

> Audit toàn diện 2026-08-29 phát hiện ~1.500 dòng chết đã xác minh + nhiều trùng lặp. Báo cáo đầy đủ: https://claude.ai/code/artifact/724fdfec-7996-40c2-ace9-335216a8268d
> **Giai đoạn 1 — XONG** (`LINT-GATE-0829` ở trên).
> **Giai đoạn 2 — XONG 2026-08-29** (4 commit `6762d82`, `2e2f959`, `30f7634`, `0289667`; 47 file, net **−520 dòng**; chi tiết HISTORY.md 2026-08-29 (3)). Đã làm: `/health` trùng, log dev `/api` trùng, gỡ gói `pg`+`@types/pg`, 4 method chết `errorTracking`, 14 khối `SelectButton`/`applyWeek`, 88 import thừa ở 33 file, `AllowanceInput`/`AllowanceRow`, bản sao `toggleAllowSellOutOfStock`, khoá `Card: 'Thẻ'`, gộp `getErrorMessage` 6 nơi → `routes/errorMessage.ts`. Cảnh báo `no-unused-vars` 184 → 79. **Chưa deploy.**
> **Còn nợ từ Giai đoạn 2** (cố ý chưa làm): 79 cảnh báo `no-unused-vars` còn lại là **biến local** (~57) và **tham số/prop destructure** (~22). Nhóm prop cần soi tay vì có thể lộ tính năng chưa nối dây — vd `SupplierListPage.tsx:43` nhận `onDeleteSupplier` từ `SupplierContainer.tsx:675` nhưng không gọi (trang tự có đường xoá riêng), `SourceDetailPage.tsx:42-43` nhận `onShopChange`/`onBack` không dùng, `TopNav.tsx:81` nhận `onImportRefresh` không dùng. Xoá mù là che mất bug thật.
...
> **ĐÃ VERIFY DB 2026-09-18**: dev **CÓ** cột `discount_percent` (`numeric`) — deploy dev hôm nay chạy bình thường. Prod **KHÔNG CÓ** (`select ... information_schema.columns` trả `(0 rows)`). Không cần chạy migration tay: `deploy-imac.sh` gọi `apply-migrations.sh --prod` ở Bước 1.6 trước khi build, `set -e` chặn nếu lỗi, và `043` là `ADD COLUMN IF NOT EXISTS`. **Chỉ còn chờ user cho phép deploy prod.**
> **(lịch sử) CÒN LẠI — BỊ CHẶN**: chưa verify được cột thật trên DB vì iMac không tới được (SSH LAN `192.168.1.2` ping mất 100%, và cả 4 hostname `dev`/`cfobrain`/`app`/`supabase-dev` đều trả **HTTP 530 error 1033** — Cloudflare Tunnel xuống, gồm cả prod). Cần khi iMac online lại:
...
> Đã phát hiện + sửa bug: Cloudflare cache `service-worker.js` 4h khiến deploy không thấy hiệu lực ngay (chi tiết HISTORY.md 2026-08-23 (5)). Đã sửa `server.ts` + deploy dev, đã thêm Cloudflare Cache Rule "Bypass cache - service worker." (áp dụng toàn zone `phucsang.com.vn`, nên prod đã được fix ở tầng CDN dù chưa deploy code).
> **Còn thiếu**: deploy `server.ts` lên prod (`scripts/deploy-imac.sh`) để origin prod cũng tự trả `Cache-Control: no-cache` — phòng trường hợp Cache Rule Cloudflare bị gỡ/đổi sau này thì vẫn có lớp bảo vệ ở code.
> Không urgent (Cache Rule đã che chắn phần hiển thị) — làm khi có dịp deploy prod tiếp theo, không cần đợt riêng.

### [x] 🟢 PURCHASE-FORM-STYLE-0823 — Đồng bộ style + tách 2 ô Bento "Phiếu nhập hàng" (GoodsPurchaseForm.tsx) *(xong 2026-08-23, đã deploy dev — chưa prod)*

> User phản hồi trang "Phiếu nhập hàng" không cùng style với trang khác, sau đó muốn tách rõ 2 khối (bảng sản phẩm / thông tin phiếu) kiểu bento thay vì dính liền nhau.
> **Đã làm**: (1) đồng bộ toàn bộ class Tailwind theo mẫu `GoodsPurchaseReturnForm.tsx` (bỏ màu hex, `font-normal` nhất quán, `rounded-xl`, border tông slate...). (2) Tách bố cục: header full-width trên cùng, bên dưới 2 card `rounded-2xl border shadow-sm` riêng biệt có `gap-4 p-4` — dựng mockup tĩnh cho user duyệt trước khi code.
> Chi tiết: HISTORY.md 2026-08-23 (2) và (3).
> `tsc` sạch, `npm test` 448/448 pass. **Đã verify bằng browser thật** (dev server local, click qua Mua hàng → Nhập hàng → Phiếu nhập hàng, xác nhận layout + nút thu gọn sidebar hoạt động đúng). Đã deploy dev — chưa deploy prod, chờ user xác nhận.
...
> User xác nhận không muốn thêm "Ngô Thành Du" (chủ cửa hàng) vào bảng `employees` vì sẽ làm lệch KPI/xếp hạng/lương của nhân viên thật (`calculateMarketingPerformance`, `calculateStaffRanking` đều duyệt qua bảng `employees`, không có cờ loại trừ).
> **Đã sửa**: thêm hằng số `OWNER_STAFF_ID`/`OWNER_STAFF_NAME` (`components/shared/staff.ts`) — KHÔNG lưu vào bảng `employees`. `POSComputer.tsx`: `salespersonOptions` giờ gồm `[Chủ cửa hàng, ...activeEmployees]`; effect tự chọn khi chưa có `currentStaffId` đổi từ "gán về nhân viên đầu tiên" sang mặc định `OWNER_STAFF_ID`. `src/lib/posSalesAttribution.ts`: `buildPosSalesRecordsForDate`/`buildPosSalesRecordUpsertsForDate` (ghi bảng `sales_records` dùng cho KPI/lương) lọc bỏ các dòng `employeeId === OWNER_STAFF_ID` trước khi ghi — báo cáo cuối ngày (`EndOfDayReport`) vẫn hiển thị đúng dòng "Chủ cửa hàng" vì gọi `calculateStaffSalesForDate` trực tiếp (không qua bộ lọc này).
> **Verify thật trên dev**: xóa `localStorage` staff cũ, reload `/pos` → mặc định hiện đúng "Chủ cửa hàng" (không còn tự gán Phạm Thị Vui). Mở dropdown → đúng 2 lựa chọn "Chủ cửa hàng" + "PHẠM THỊ VUI" (Cẩm Tú không hiện vì đã nghỉ việc, đúng lọc `activeEmployees`). Chọn "PHẠM THỊ VUI" → header cập nhật đúng, `localStorage['cfo_current_staff_id']` lưu đúng ID thật `EMP-441676`.
> `npm test` 448/448 sạch, `tsc --noEmit` sạch. Đã cập nhật `docs/business-knowledge/FORMULAS.md` mục 5.3 ghi rõ quy tắc loại trừ `OWNER_STAFF_ID` khỏi KPI. Đã deploy dev.
> Files: `components/shared/staff.ts`, `components/pos/POSComputer.tsx`, `src/lib/posSalesAttribution.ts`, `docs/business-knowledge/FORMULAS.md`.
> **Chưa đụng prod** — chờ user yêu cầu riêng theo quy tắc mặc định chỉ làm trên dev.

### [x] 🔴🔴 SYNC-FORCE-RESURRECT-0819 — "Sync Force" hồi sinh dữ liệu đã xóa lên server, kể cả TỰ ĐỘNG khi mạng reconnect *(xong 2026-08-19, dev — bước 1+2 hoàn tất, verify thật)*

> **Bối cảnh phát hiện**: Đang test mục 4 checklist (bán nợ đơn thứ 2 cho cùng khách) — trang Khách hàng hiện "Nợ hiện tại" của A ANH = 185.000đ trong khi SQL xác nhận `customer_debt_history` thật chỉ có 35.000đ (2 đơn nợ vừa test). Bấm nút "Tải lại dữ liệu và đồng bộ database" để điều tra → phát hiện SQL sau đó cho thấy 1 đơn hàng đã xóa từ phiên trước (`HD-SXR7W`, 150.000đ) **xuất hiện trở lại trong DB thật**, `created_at` mới đúng thời điểm bấm nút, không có dòng nào trong `audit_logs` ghi nhận việc "tái tạo" này.
> **Nguyên nhân xác nhận qua đọc code** (`hooks/useAppData.ts:539-643`, khối gọi là "Sync Force"): khi `fetchData(isManual=true, ...)` chạy (nút "Tải lại dữ liệu" ở TopNav/SettingsCenter/MainContent, HOẶC tự động qua `silentSync` — `useAppData.ts:1395` — gắn vào sự kiện `window.addEventListener('online', ...)` ở `App.tsx:172`, chạy **hoàn toàn im lặng mỗi khi mạng reconnect, không cần ai bấm gì**), code so ID trong cache IndexedDB cục bộ (`newState[config.key]`) với ID vừa fetch từ server (`results...`), cho **26 bảng** (kể cả `posOrders`, `customerDebtHistory`, `expenses`, `payroll`, `suppliers`, `inventoryTransactions`...). Bất kỳ ID nào "local có, server không có" bị coi là `missingOnCloud` → tự động `apiService.upsertMany(...)` đẩy thẳng lên Supabase — **không phân biệt được** "bản ghi local chưa từng gửi lên server" (hợp lệ, đúng mục đích offline-first) với "bản ghi đã có trên server rồi bị xóa" (không nên đồng bộ ngược). Không có audit log vì `pos_orders`/`customer_debt_history` không nằm trong `AUDITED_TABLES` (`routes/data.ts`).
> **Tác động thật — không giới hạn ở xóa qua SQL trực tiếp**: kịch bản hoàn toàn có thể xảy ra trong vận hành bình thường — nhân viên A xóa 1 đơn hàng đúng quy trình qua UI trên máy A, nhân viên B có tab/máy B đang mở với cache cũ (chưa fetch lại) → mạng B chập chờn rồi có lại (rất thường xảy ra ở cửa hàng) → `silentSync` tự động chạy im lặng → đơn đã xóa **sống lại trên server thật**, kể cả **prod**. Áp dụng cho tất cả 26 bảng trong `allSyncConfigs`, không riêng đơn hàng.
> **Đã tắt tạm** (`hooks/useAppData.ts` dòng ~539): thêm `const SYNC_FORCE_ENABLED: boolean = false;` chặn toàn bộ khối "Sync Force" ở cả 4 điểm gọi (TopNav/SettingsCenter/MainContent refresh + `silentSync` khi mạng reconnect). Giữ nguyên code cũ (không xóa) để tham khảo khi thiết kế lại đúng.
> **Đã verify fix**: hard-refresh browser (code mới), bấm lại đúng nút từng gây lỗi → SQL xác nhận `pos_orders`/`customer_debt_history` count không đổi, không có dòng mới.
> **Đã dọn dữ liệu ô nhiễm do bug gây ra** trước khi tắt (6 đơn hàng test cũ đã hồi sinh: `HD-SXR7W`, `HD-V17HD`, `HD-VMYFP`, `HD-W2I97`, `HD-WBEJX`, `TH-WEZ2Y` — tổng 770.000đ giả — cùng 10 `inventory_transactions` và 1 `customer_debt_history` liên quan). `revenue_records` không bị ảnh hưởng (bảng này so khớp theo `date` chứ không phải `id`, ngày đã tồn tại nên không bị coi là thiếu).
...
> **Đã sửa (bước 2/2)**: `mergeBy()` (`services/dataMapper.ts`) nhận thêm tham số `pendingIds?: Set<unknown>` — nhánh "local có, cloud không có" giờ **chỉ giữ lại** nếu bản ghi đang thực sự nằm trong offline queue (`cfo_brain_pos_queue`), không còn giữ vô điều kiện. `mapAllData()` nhận thêm `pendingIdsByKey?: Map<string, Set<unknown>>`, build từ `posOfflineQueue.getAll()` ngay tại `fetchData` (`hooks/useAppData.ts`) mỗi lần fetch — map theo đúng field khóa từng bảng (`id` mặc định, `date` cho revenue/shopeeRevenue, `sku` cho shopeeSourceData). Cả 26 lời gọi `mergeBy` trong `mapAllData` đã cập nhật truyền `pending('<key>')`. Không xóa merge logic gốc (đúng `.claude/rules/codebase.md`) — chỉ siết điều kiện giữ lại.
> **Đã verify thật trên dev**: tạo 1 phiếu chi thật qua UI Sổ quỹ (id `ea7728ca-...`, "TEST MERGEBY GUARD VERIFY", 9999đ) → xác nhận có trong `expense_records` qua SQL → xóa thẳng bằng SQL (`DELETE ... WHERE id=...`) mô phỏng "server xóa ngoài luồng trong khi cache local vẫn còn" → bấm "Tải lại dữ liệu" → reload lại trang Sổ quỹ → UI hiển thị đúng "Tổng chi: 0", bản ghi ma biến mất hoàn toàn, không hồi sinh. SQL xác nhận không còn dữ liệu test (`0 rows`).
> `npm test` sạch, `tsc --noEmit` sạch. Đã deploy dev (bundle `index-hz_IhILJ.js`).
> Files thêm: `hooks/useAppData.ts` (`updateData` ~857-880; thêm `buildPendingIdsByKey`/`MERGE_KEY_BY_DATA_KEY` sau `isRetryableSyncError`; `fetchData` build `pendingIdsByKey` từ queue trước khi gọi `mapAllData`), `services/dataMapper.ts` (`mergeBy` + `mapAllData` signature, 26 call site).
> **Còn lại (không chặn, ghi nhận)**: Sync Force (mục tự động khi reconnect) vẫn đang tắt tạm qua `SYNC_FORCE_ENABLED = false` — chưa quyết định bật lại hay bỏ hẳn, cần bàn với user riêng vì giờ queue + mergeBy đã xử lý đúng luồng ghi offline hợp lệ, có thể không cần Sync Force nữa.

### [x] 🟠 PURCHASE-NCC-UUID-0819 — Tạo phiếu nhập hàng không chọn Nhà cung cấp → lưu thất bại 100%, lỗi im lặng *(xong 2026-08-19)*

> **Bối cảnh phát hiện**: Test mục 10 checklist (nhập hàng) trên dev — tạo Phiếu nhập hàng cho 1 sản phẩm, điền SL/đơn giá đầy đủ nhưng **không chọn Nhà cung cấp** (field không bắt buộc trên UI, không có dấu `*`), bấm "Hoàn thành" → toast lỗi chung chung "Không thể ghi dữ liệu", toàn bộ thao tác rollback.
> **Nguyên nhân xác nhận qua log server** (`/tmp/cfobrain-app-dev.log`): `[DataRoute] upsert failed [supplier_debts]: invalid input syntax for type uuid: "ncc-le"`. Root cause: `components/purchase/PurchaseOrdersContainer.tsx:962` (tạo phiếu nhập) và `:1137` (trả hàng nhập) — `const debtSupplierId = supplier?.id || 'ncc-le';` — khi không chọn NCC, code dùng fallback string `'ncc-le'` làm `supplierId`. `services/apiService.ts:413` map thẳng `supplier_id: item.supplierId` khi ghi bảng `supplier_debts` (cột `supplier_id` kiểu UUID) — không validate UUID trước khi gửi, Postgres từ chối cả request (`22P02`).
...
> **Bối cảnh phát hiện**: Sau khi xóa sạch cache trình duyệt (IndexedDB + localStorage) để verify fix `mergeBy` (xem `SYNC-FORCE-RESURRECT-0819`), load lại POS → "HỆ THỐNG CHƯA CÓ SẢN PHẨM" dù DB dev có đủ 15.015 sản phẩm. Console báo `Đồng bộ quá thời gian — kiểm tra kết nối mạng` — tái hiện lại **3/3 lần thử liên tiếp** với timeout cũ 45s.
> **Nguyên nhân xác nhận**: `services/apiService.ts` khai báo `SUPABASE_PAGE_SIZE = 5000` cho `fetchAllRows()` (dùng để phân trang `pos_products`), nhưng server PostgREST dev thực tế giới hạn cứng `PGRST_DB_MAX_ROWS=1000` (xác nhận qua `docker inspect supabase-rest-dev`) — mỗi trang chỉ nhận tối đa 1000 dòng bất kể code xin 5000. Riêng `pos_products` cần **16 lượt gọi tuần tự** thay vì 4 lượt như code định, cộng với `pos_orders` (cửa sổ 60 ngày) và ~25 bảng khác chạy song song trong cùng `Promise.all`, tổng thời gian cold-sync (không có cache để dùng tạm) dễ vượt quá 45s.
> **Đã sửa tạm**: tăng `FETCH_TIMEOUT_MS` trong `hooks/useAppData.ts` từ 45.000 lên 120.000 (120s) — đủ thời gian cho cold-sync catalog lớn hoàn tất. Verify thật: xóa cache, reload, đợi ~100s → cache IndexedDB xác nhận đủ dữ liệu (`posProducts: 15015, posOrders: 1157, employees: 2`), search sản phẩm trong POS hoạt động bình thường sau đó.
...
> **Bối cảnh phát hiện**: User yêu cầu tự test thực tế theo checklist `docs/06-evaluation/CHECKLIST_TEST_THUC_TE_THAY_KIOTVIET.md` (không dựa vào báo cáo cũ). Tạo SP/khách hàng test trên dev, phát hiện tạo khách hàng lỗi 500 âm thầm (UI báo thành công, network thực tế `PGRST204: Could not find the 'debt_amount' column`).
> **Xác minh bằng chứng đầy đủ**: (1) `information_schema.columns` xác nhận bảng `pos_customers` **không có cột `debt_amount`** trên CẢ dev VÀ **prod** dù `supabase_setup.sql:901` có ghi ALTER TABLE thêm cột này (chưa từng thực sự chạy). (2) RPC `delete_pos_order_tx` (từ migration `030`) có dòng `UPDATE pos_customers SET ... debt_amount = ...` chạy vô điều kiện mỗi khi đơn có `customer_id`. (3) Tái hiện thật: đặt đơn bán nợ → xóa đơn → 500 `column "debt_amount" does not exist | 42703`.
> **Đã sửa (migration 038)**: bỏ dòng `debt_amount = ...` khỏi `UPDATE pos_customers` trong `delete_pos_order_tx` (cột tàn dư, nguồn sự thật duy nhất là `customer_debt_history` theo thiết kế đã chốt 2026-08-14) + bỏ field `debt_amount` khỏi payload `services/apiService.ts:139` (`sanitizeItem` cho `posCustomers`).
> **2 bug ẩn phát hiện thêm khi verify fix (cùng function, bị che bởi lỗi debt_amount nên chưa từng lộ ra)**:
> - **Migration 039**: `delete_pos_order_tx` khai báo `RETURNS TABLE (id UUID)` tạo biến PL/pgSQL tên `id` trùng cột `id` của `pos_products`/`inventory_transactions`/`pos_customers`/`pos_orders` — các câu `UPDATE ... WHERE id = ...` không qualify tên bảng bị Postgres báo `column reference "id" is ambiguous (42702)`. Đã qualify tên bảng cho tất cả (`pos_products.id`, `inventory_transactions.id`, `pos_customers.id`, `pos_orders.id`).
> - **Migration 040**: Vòng lặp hoàn tồn kho dùng `status <> 'cancelled'` — nhưng `inventory_transactions` loại `Sale` có `status` mặc định **NULL**, và `NULL <> 'cancelled'` cho kết quả NULL (không phải TRUE) theo logic 3 giá trị SQL → dòng NULL bị loại khỏi vòng lặp, **tồn kho không bao giờ được hoàn lại khi xóa đơn** (dù hệ thống báo xóa thành công). Đã đổi thành `COALESCE(status, '') <> 'cancelled'` (đúng pattern đã dùng sẵn ở `cancel_pos_return_tx`/`edit_pos_order_tx`).
> **Verify thật trên dev (vòng cuối, sau cả 3 migration)**: bán 1 SP thật (không cần khách hàng) → thanh toán → xóa đơn qua UI → SQL xác nhận: `pos_orders.status='cancelled'`, `inventory_transactions.status='cancelled'` (trước đó luôn là NULL), tồn kho hoàn đúng về giá trị gốc. Riêng test đầu (bán nợ cho khách "A ANH") cũng verify: đơn xóa thành công, `customer_debt_history` xóa sạch, khách sạch nợ.
...
> User báo không xoá được nội dung ô trong 4 bảng nhập liệu lương. Kiểm tra phát hiện **2 bug riêng biệt**:
> **Bug A (Chấm công)**: popup chọn nhanh trong `AttendanceTab.tsx` — hàm `commitValue()` khi nhận chuỗi rỗng chỉ đóng popup rồi `return`, không hề gọi `onChange` xuống handler xoá. 100% không xoá được, mọi trường hợp.
> **Bug B (Tăng ca/Doanh số/Khấu trừ)**: xoá vẫn hoạt động đúng (đi qua `/api/data/delete`, xoá đúng 1 dòng), nhưng **thêm/sửa** 1 ô lại gửi lên NGUYÊN MẢNG cả bảng qua `/api/data/upsert-many` — nếu nhân viên có bất kỳ tháng nào khác đã chốt lương lẫn trong mảng, cơ chế khoá lương (thêm hồi sáng cùng ngày, PAYROLL-LOCK-0805) chặn nhầm **cả request**, dù ô đang sửa thuộc tháng chưa chốt. Giao diện vẫn hiện giá trị vừa gõ do cache lạc quan, khiến tưởng đã lưu nhưng thực ra không xuống DB — lỗi im lặng.
...
> User phản ánh "Sổ cái lương" trống vì mặc định lọc theo tháng hiện tại (chưa ai chốt lương tháng đó) — sau khi giải thích, user yêu cầu đổi hẳn: Sổ cái lương hiện **tất cả các tháng** (giống Sổ cái hiệu năng), breakdown đầy đủ 15 field lương để phân tích; Sổ cái hiệu năng chỉ giữ doanh số (bỏ cột liên quan lương).
