...
> **Còn sót (không chặn, mức thấp hơn — app runtime code)**: `server.ts` (3 chỗ: CSP connectSrc, 2 fallback URL local Kong `192.168.1.6:8000/8010`) + `routes/channelManagement.ts:57` vẫn còn IP cũ trong nhánh **auto-detect LAN fallback** (chỉ dùng khi dev chạy gần iMac, có fallback khác phía sau nếu fail — không downtime). Cần restart+verify browser theo quy tắc sửa code app (khác nhóm script infra thuần bash vừa sửa) → để riêng, chưa làm trong phiên này.
