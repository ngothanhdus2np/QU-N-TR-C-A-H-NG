...
### C.5 — Khi user yêu cầu "thêm thông tin" cho một trang/tab/chức năng
1. Hiểu mặc định là thêm **nút icon thông tin** (`Info`/`HelpCircle`) ở gần cụm hành động chính của trang/tab, thường cạnh nút `Xuất file` hoặc nhóm nút tạo phiếu.
2. Bấm nút mở popup/modal nhỏ, không dùng tooltip nếu nội dung dài hơn một câu.
3. Nội dung bắt buộc gồm:
   - Tên chức năng/tab đang giới thiệu.
   - Chức năng này dùng để làm gì.
   - Các thao tác chính người dùng cần biết.
   - Lưu ý nghiệp vụ quan trọng, đặc biệt nếu bấm hoàn thành/lưu sẽ cập nhật tồn kho, tiền, công nợ, lương, hoặc dữ liệu tài chính.
4. Viết ngắn gọn, theo ngôn ngữ người dùng cuối; không mô tả kỹ thuật, không nói về implementation.
5. Giữ design system hiện tại: icon từ `lucide-react`, modal/popup bo góc, nền trắng, chữ slate/indigo, nút đóng rõ ràng.
6. Nếu cùng trang có nhiều luồng nghiệp vụ, nội dung phải phân biệt từng luồng bằng tiêu đề ngắn, ví dụ `Phiếu kiểm kho` và `Phiếu trừ hàng lỗi/hư`.
...
**Typography rule áp dụng toàn app:**
- Chỉ dùng chữ in đậm (`font-bold`/`font-black`) cho **tiêu đề mục/section** và **tiêu đề cột bảng**.
- Nội dung còn lại — label, mô tả, input/select, button, tab, badge, dữ liệu bảng, pagination, footer, helper text — dùng font thường (`font-normal`) hoặc bỏ class font-weight.
- Không dùng in đậm để làm nổi bật dữ liệu thường; nếu cần nhấn trạng thái thì dùng màu, icon, spacing hoặc badge nhẹ theo design system.
- Khi sửa UI cũ, nếu đụng vào khu vực đang có chữ đậm sai rule này thì chỉnh scoped trong khu vực đó, không format/refactor toàn repo.
