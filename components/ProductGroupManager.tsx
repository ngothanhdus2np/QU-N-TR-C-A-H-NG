...
import { 
  ...
  History, Boxes, Award, ArrowUp, Briefcase
} from 'lucide-react';
import { calculateStrategicSuggestions, calculateSeasonalityAnalysis, cleanVNNumber, parseVNDate, parseHierarchyGroups, generateId, processExcelRawData, isUUID, type ParsedExcelRow } from '../src/lib';
...
const ProductGroupManager: React.FC<Props> = ({ productGroups, products = [], groupRevenue, onUpdateGroups, onUpdateGroupRevenue }) => {
  ...
  return (
    <div className="space-y-8 pb-20 max-w-full animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div className="flex items-center gap-5">
              ...
              <div>
                 ...
                 <p className="text-[11px] text-indigo-600 font-normal uppercase tracking-widest mt-1">Tìm quy luật bán hàng qua nhiều năm</p>
              </div>
           </div>
           
           <div className="flex items-center gap-4 bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shadow-inner">
              <button onClick={() => setViewMode('quantity')} className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-normal uppercase tracking-widest transition-all ${viewMode === 'quantity' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
                <Boxes className="w-4 h-4" /> Số lượng
              </button>
              <button onClick={() => setViewMode('revenue')} className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-normal uppercase tracking-widest transition-all ${viewMode === 'revenue' ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-[2rem] w-fit mx-auto shadow-sm border border-slate-200">
        <button onClick={() => changeSubTab('groups')} className={`flex items-center gap-2 px-8 py-4 rounded-[1.5rem] font-normal text-[10px] uppercase tracking-widest transition-all ${activeSubTab === 'groups' ? 'bg-white text-blue-600 shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}>
          <Boxes className="w-4 h-4" /> Nhóm Hàng
        </button>
        <button onClick={() => changeSubTab('seasonality')} className={`flex items-center gap-2 px-8 py-4 rounded-[1.5rem] font-normal text-[10px] uppercase tracking-widest transition-all ${activeSubTab === 'seasonality' ? 'bg-white text-emerald-600 shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}>
          <Sparkles className="w-4 h-4" /> Dự Báo Chiến Lược
        </button>
        <button onClick={() => changeSubTab('matrix')} className={`flex items-center gap-2 px-8 py-4 rounded-[1.5rem] font-normal text-[10px] uppercase tracking-widest transition-all ${activeSubTab === 'matrix' ? 'bg-white text-indigo-600 shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}>
          <Filter className="w-4 h-4" /> Ma Trận Phân Tích Năm
        </button>
        <button onClick={() => changeSubTab('ledger')} className={`flex items-center gap-2 px-8 py-4 rounded-[1.5rem] font-normal text-[10px] uppercase tracking-widest transition-all ${activeSubTab === 'ledger' ? 'bg-white text-blue-600 shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}>
                 ...
                 <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] text-emerald-600 font-normal uppercase tracking-widest">Xếp hạng thực tế theo {viewMode === 'revenue' ? 'Doanh thu' : 'Số lượng'}</p>
                       </div>
                    </div>
                 </div>
                       ...
                       <div key={g.name} className="space-y-2 group">
                          <div className="flex justify-between items-end">
                             <div className="flex items-center gap-3">
                                <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-normal ${idx === 0 ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}>{idx + 1}</span>
                                <span className="text-sm font-normal text-slate-800 uppercase truncate max-w-[200px]">{g.name}</span>
                                {idx === 0 && <span className="px-2 py-0.5 bg-amber-100 text-amber-600 rounded text-[8px] font-normal uppercase flex items-center gap-1"><Award className="w-2.5 h-2.5" /> Best Seller</span>}
                             </div>
                             <div className="text-right">
                                <span className="text-xs font-normal text-emerald-700">{g.percent.toFixed(1)}%</span>
                             </div>
                          </div>
                          ...
                          <p className="text-[9px] font-normal text-slate-400 uppercase tracking-tighter">Đạt: {viewMode === 'revenue' ? formatNumber(g.rev) + 'đ' : formatNumber(g.qty) + ' đv'}</p>
                       </div>
                    ))}
                 </div>
              </div>
                 ...
                 <div className="flex items-center justify-between mb-10 relative z-10">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] text-indigo-600 font-normal uppercase tracking-widest">Dự báo dựa trên TB lịch sử {years.length} năm</p>
                       </div>
                    </div>
                 </div>
                       ...
                       <div key={g.name} className="p-5 bg-indigo-50/30 border border-indigo-100/50 rounded-[1.5rem] hover:bg-white hover:shadow-lg transition-all group">
                          <div className="flex justify-between items-start mb-3">
                             <div className="flex items-center gap-3">
                                ...
                                <span className="text-xs font-normal text-slate-800 uppercase truncate max-w-[180px]">{g.name}</span>
                             </div>
                             <div className="text-right">
                          ...
                          <div className="flex items-center justify-between">
                             <div className="space-y-1">
                                <p className="text-[9px] font-normal text-slate-400 uppercase tracking-widest">Dự báo hiệu suất</p>
                                <p className="text-sm font-normal text-indigo-700">{viewMode === 'revenue' ? formatNumber(g.avgRev) + 'đ' : formatNumber(g.avgQty) + ' đv'}</p>
                             </div>
              ...
              <div className="lg:col-span-10 bg-slate-900 p-10 md:p-14 rounded-[4rem] text-white shadow-2xl relative overflow-hidden">
                 ...
                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 relative z-10">
                    <div>
                       ...
                       <p className="text-slate-400 text-lg mt-4 max-w-2xl font-normal">Đối soát sự chuyển dịch thị trường từ Tháng {selectedMonthNum} sang Tháng {nextMonthNum} để tối ưu kho vận.</p>
                    </div>
                    <button onClick={runStrategicDiagnosis} disabled={isDiagnosing} className="px-12 py-7 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[2.5rem] font-normal text-xs uppercase tracking-[0.2em] shadow-2xl flex items-center justify-center gap-4 active:scale-95 transition-all disabled:opacity-50">
            ...
            <div className="p-10 border-b border-slate-100 bg-indigo-50 flex items-center justify-between">
              <div className="flex items-center gap-5">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-indigo-600 font-normal uppercase tracking-widest mt-1">Phát hiện cấu trúc mới</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
              <div className="bg-amber-50 p-6 rounded-[2rem] border-2 border-dashed border-amber-200 flex items-start gap-5">
                 ...
                 <div>
                    ...
                    <p className="text-sm text-amber-800 mt-1 font-normal">Tìm thấy <strong>{pendingGroupRevenue.length}</strong> dòng doanh thu.</p>
                 </div>
              </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[200px] overflow-y-auto pr-2">
                      {pendingGroups.map((g, idx) => (
                        <div key={idx} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                           ...
                           <div><p className="text-[10px] font-normal text-slate-400 uppercase">Cấp {g.level}</p><p className="text-xs font-normal text-slate-800 truncate">{g.fullPath}</p></div>
                        </div>
                      ))}
                   </div>
                </div>
              ...
              {groupConflicts.length > 0 && (
                <div className="space-y-6">
                   ...
                   <div className="bg-rose-50 p-6 rounded-3xl border border-rose-100 space-y-4">
                      <p className="text-xs font-normal text-rose-800 uppercase tracking-tight">Chọn phương án xử lý trùng lặp:</p>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {[
                          ...
                          >
                            <p className={`text-[10px] font-normal uppercase ${conflictResolution === opt.id ? 'text-rose-600' : 'text-slate-400'}`}>{opt.label}</p>
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {groupConflicts.map((c, i) => (
                            <tr key={i} className="hover:bg-slate-50">
                              ...
                              <td className="p-3 font-normal truncate max-w-[150px]">{c.groupName}</td>
                              <td className="p-3 text-right">{formatNumber(c.currentAmount)}</td>
                              <td className="p-3 text-right font-normal text-rose-600">{formatNumber(c.newAmount)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white flex items-center justify-between">
                 <div className="flex items-center gap-6">
                    <div className="text-center">
                       <p className="text-[10px] font-normal text-slate-400 uppercase mb-1">Tổng bản ghi</p>
                       <p className="text-2xl font-normal">{pendingGroupRevenue.length}</p>
                    </div>
                    <div className="text-center">
                       <p className="text-[10px] font-normal text-slate-400 uppercase mb-1">Tổng doanh thu</p>
                       <p className="text-2xl font-normal text-emerald-400">{formatNumber(pendingGroupRevenue.reduce((s,r) => s + (r.amount || 0), 0))}đ</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-3">
                    <span className="text-[10px] font-normal text-slate-400 uppercase">Tháng hạch toán:</span>
                    <input type="month" value={syncAccountingMonth} onChange={e => setSyncAccountingMonth(e.target.value)} className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 text-xs font-normal text-white" />
                 </div>
              </div>
            </div>

            <div className="p-10 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
               <button onClick={() => setIsAuditMode(false)} className="px-10 py-5 rounded-2xl font-normal text-xs text-slate-400 uppercase tracking-widest">Hủy bỏ</button>
               <button onClick={handleCommitSync} disabled={isCommitting} className="px-14 py-5 bg-indigo-600 text-white rounded-2xl font-normal text-xs uppercase tracking-widest shadow-2xl flex items-center gap-4 transition-all disabled:opacity-50">
