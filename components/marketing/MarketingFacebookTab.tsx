    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h3 className="text-sm font-semibold uppercase text-slate-800">Cấu hình & Kết nối</h3>
        </div>

        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-2xs font-normal text-slate-400 uppercase ml-1">App ID</label>
            <input
              type="text"
          ...
          <div className="space-y-1.5">
            <label className="text-2xs font-normal text-slate-400 uppercase ml-1">App Secret</label>
            <input
              type="password"
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={fbLoading}
              className="flex-1 py-4 bg-slate-900 text-white rounded-xl font-normal text-2xs uppercase hover:bg-slate-800 transition-colors disabled:opacity-50"
            >
            ...
            {!isFbConnected ? (
              <button
                ...
                className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-normal text-2xs uppercase shadow-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
              ...
              <div className="flex-1 flex gap-2">
                <button
                  ...
                  disabled={fbLoading}
                  className="flex-1 py-4 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-xl font-normal text-2xs uppercase hover:bg-indigo-100 transition-colors flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="flex-1 py-4 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl font-normal text-2xs uppercase hover:bg-rose-100 transition-colors"
                >
                  Ngắt kết nối
                </button>
              </div>
        ...
        {isFbConnected && (
          <div className="p-4 bg-green-50 border border-green-100 rounded-2xl flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-2xs font-normal text-green-700 uppercase">Tài khoản đã sẵn sàng</span>
            </div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold uppercase text-slate-800">Danh sách Fanpage</h3>
          </div>
          {isFbConnected && (
            <button
              onClick={fetchFbPages}
        ...
        {!isFbConnected ? (
          <div className="flex flex-col items-center justify-center py-12 gap-4 text-center">
            ...
            <p className="text-2xs text-slate-400 font-normal uppercase max-w-[200px] leading-relaxed">
              Vui lòng kết nối Facebook ở cột bên trái để xem danh sách Fanpage.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <label className="text-2xs font-normal text-slate-400 uppercase ml-1">Chọn Fanpage để đăng bài</label>
            <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {fbPages.length > 0 ? (
                fbPages.map((page) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-normal text-slate-800 uppercase">{page.name}</span>
                ...
                <div className="flex flex-col items-center gap-3 py-10 text-center">
                  <p className="text-2xs font-normal text-slate-400 uppercase">Không tìm thấy Fanpage nào</p>
            ...
            {isFbConnected && selectedPageId && (
              <div className="pt-4 border-t border-slate-100 space-y-4">
                <div className="flex items-center justify-between p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                  <div className="flex flex-col">
                    <span className="text-2xs font-normal text-indigo-900 uppercase">Tự động đăng bài</span>
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full transition-colors relative ${autoPostConfig.enabled ? 'bg-indigo-600' : 'bg-slate-300'}`}
                  >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-colors ${autoPostConfig.enabled ? 'left-7' : 'left-1'}`}></div>
                  </button>
                </div>
