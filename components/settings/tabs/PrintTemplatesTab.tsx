...
// Gộp 30/08/2026: trước đó file này tự viết lại nguyên bảng Code128 + hàm sinh SVG,
// không hề biết tới util dùng chung. Truyền height:50/ariaLabel:false để giữ đúng
// đầu ra cũ của file này (mặc định của util là 40 + có aria-label).
import { buildCode128Svg } from '../../pos/goods/barcodeUtils';
...
const PrintTemplatesTab: React.FC<PrintTemplatesTabProps> = ({ brandProfile }) => {
  ...
  const barcodeLabelPrintCss = useMemo(
    () => `
    @page { size: ${barcodeLabelTotalWidthMm}mm ${barcodeLabelHeightMm}mm; margin: 0; }
  ...
  const renderBarcodeLabelHtml = useCallback(
    (product: { name: string; sku: string; price: string }) =>
      `<section class="label">${barcodeLabelShowName ? `<div class="name">${escapeHtml(product.name)}</div>` : ''}${buildCode128Svg(product.sku, { height: 50, ariaLabel: false })}${barcodeLabelShowCode ? `<div class="code">${escapeHtml(product.sku)}</div>` : ''}${barcodeLabelShowPrice ? `<div class="price">${escapeHtml(product.price)} VNĐ</div>` : ''}</section>`,
