...
const MatrixTab: React.FC<Props> = ({
  ...
  // Pre-group theo groupId + lọc date range 1 lần duy nhất — tránh O(groups × rows) mỗi render
  const groupRevMap = useMemo(() => {
    const map = new Map<string, { qty: number; retQty: number; net: number; cogs: number }>();
    for (const r of groupRevenue) {
      if (r.date < timeContext.start || r.date > timeContext.end) continue;
      const key = r.groupId;
      const cur = map.get(key) ?? { qty: 0, retQty: 0, net: 0, cogs: 0 };
      cur.qty    += r.quantity        || 0;
      cur.retQty += r.returnsQuantity || 0;
      cur.net    += r.netRevenue      || 0;
      cur.cogs   += r.cogs            || 0;
      map.set(key, cur);
    }
    return map;
  }, [groupRevenue, timeContext.start, timeContext.end]);
  ...
  <div className="bg-white rounded-[3.5rem] p-8 md:p-12 border border-slate-200 shadow-xl flex flex-col gap-10">
    <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-slate-100 pb-10">
      ...
      <div className="px-6 py-3 bg-slate-50 rounded-2xl border border-slate-100">
        <span className="text-2xs font-normal text-slate-400 uppercase tracking-widest">Giai đoạn: </span>
        ...
        <table className="w-full border-collapse">
          ...
          <tbody className="divide-y divide-slate-50">
            {productGroups.map(group => {
              const { qty = 0, retQty = 0, net = 0, cogs = 0 } = groupRevMap.get(group.id) ?? {};
              return (
                <tr key={group.id} className="hover:bg-slate-50 transition-colors">
                  <td className="p-6 font-normal text-slate-700 uppercase text-xs">{group.name}</td>
                  <td className="p-6 text-right tabular-nums font-normal text-blue-600">{qty.toLocaleString()}</td>
                  <td className="p-6 text-right tabular-nums font-normal text-rose-500">{retQty.toLocaleString()}</td>
                  <td className="p-6 text-right tabular-nums font-normal text-emerald-600">{net.toLocaleString()}đ</td>
                  <td className="p-6 text-right tabular-nums font-normal text-slate-500">{cogs.toLocaleString()}đ</td>
                </tr>
              );
            })}
          </tbody>
            ...
            {financialMatrixData.map((row) => (
              <tr key={row.key} className="group hover:bg-slate-50/80 transition-colors">
                ...
                {years.map(y => {
                  ...
                  return <td key={y} className={`p-5 text-center tabular-nums text-xs border-r border-slate-50 transition-colors ${getHeatmapColor(val)}`}>{val !== 0 ? formatNumber(val) : '0'}</td>;
                })}
              </tr>
            ))}
          </tbody>
