    ...
    <div className="flex items-center justify-between border-b border-slate-100 pb-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-normal uppercase tracking-widest">Ghi nhận lịch sử nhập hàng hóa</p>
        </div>
      </div>
      <button className="px-6 py-3 bg-sky-600 text-white rounded-xl font-normal text-[10px] uppercase tracking-widest shadow-lg flex items-center gap-2">
        ...
        <tbody className="divide-y divide-slate-50 text-xs font-normal">
          {shopeeInventoryIn.length > 0 ? shopeeInventoryIn.map(item => (
            <tr key={item.id} className="hover:bg-slate-50 transition-all">
              ...
              <td className="p-6 text-slate-600 font-normal uppercase">{item.sku}</td>
              ...
              <td className="p-6 text-right font-normal text-slate-900">{formatNumber(item.quantity * item.importPrice)}đ</td>
              <td className="p-6 text-slate-400 italic">{item.note || '-'}</td>
            </tr>
          )) : (
            <tr>
              <td colSpan={6} className="p-20 text-center text-slate-400 font-normal uppercase tracking-widest">Chưa có lịch sử nhập kho</td>
            </tr>
          )}
        </tbody>
      </table>
