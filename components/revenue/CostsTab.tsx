            ...
            <tbody className="divide-y divide-slate-50">
              {(shopeeCosts?.fixedCosts || []).map((item, idx) => (
                <tr key={item.id} className="group">
                  ...
                  <td className="py-4"><input type="text" value={item.name} onChange={(e) => handleUpdateShopeeCostItem('fixed', item.id, 'name', e.target.value)} className="w-full bg-transparent border-none outline-none text-xs font-bold text-slate-700 focus:text-indigo-600" /></td>
                  <td className="py-4"><input type="number" value={item.quantity} onChange={(e) => handleUpdateShopeeCostItem('fixed', item.id, 'quantity', Number(e.target.value))} className="w-full bg-transparent border-none outline-none text-xs font-bold text-center text-slate-600" /></td>
                  <td className="py-4"><input type="number" value={item.unitPrice} onChange={(e) => handleUpdateShopeeCostItem('fixed', item.id, 'unitPrice', Number(e.target.value))} className="w-full bg-transparent border-none outline-none text-xs font-bold text-right text-slate-600" /></td>
                  <td className="py-4 text-right text-xs font-black text-slate-900">{formatNumber(item.quantity * item.unitPrice)}</td>
        ...
        <div className="mt-auto pt-8 border-t border-slate-100 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
              <input type="number" value={shopeeCosts?.targetOrders || 0} onChange={(e) => handleUpdateShopeeCostConfig({ targetOrders: Number(e.target.value) })} className="w-16 bg-transparent border-none outline-none text-sm font-black text-indigo-600 text-center" />
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {(shopeeCosts?.variableCosts || []).map((item, idx) => (
                <tr key={item.id} className="group">
                  <td className="py-4 text-[10px] font-black text-slate-300">{idx + 1}</td>
                  <td className="py-4"><input type="text" value={item.name} onChange={(e) => handleUpdateShopeeCostItem('variable', item.id, 'name', e.target.value)} className="w-full bg-transparent border-none outline-none text-xs font-bold text-slate-700 focus:text-rose-600" /></td>
                  <td className="py-4"><input type="number" value={item.quantity} onChange={(e) => handleUpdateShopeeCostItem('variable', item.id, 'quantity', Number(e.target.value))} className="w-full bg-transparent border-none outline-none text-xs font-bold text-center text-slate-600" /></td>
                  <td className="py-4"><input type="number" value={item.unitPrice} onChange={(e) => handleUpdateShopeeCostItem('variable', item.id, 'unitPrice', Number(e.target.value))} className="w-full bg-transparent border-none outline-none text-xs font-bold text-right text-slate-600" /></td>
                  <td className="py-4 text-right text-xs font-black text-slate-900">{formatNumber(item.quantity * item.unitPrice)}</td>
