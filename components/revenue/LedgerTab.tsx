      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl">
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <InputWrapper label="Thời gian" icon={Calendar}>
              <input
                type="date"
                required
                value={formData.date}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <InputWrapper label="Giá trị trả" icon={ArrowRightLeft} color="text-amber-500">
              <input
                type="number"
                placeholder="0"
    ...
    <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl overflow-hidden relative flex flex-col max-h-[800px]">
      <div className="px-8 py-6 bg-slate-50 border-b border-slate-100 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 sticky top-0 z-20">
        ...
        <div className="px-6 py-2.5 bg-white border border-slate-200 rounded-xl font-normal text-[9px] uppercase tracking-widest">
          Dữ liệu: {timeContext.start.split('-').reverse().join('/')} →{' '}
          ...
          <tbody className="divide-y divide-slate-50 tabular-nums text-[11px] font-normal">
            {filteredListByRange.map(item => (
              ...
              >
                ...
                <td className="px-1 py-5 text-right text-blue-600">
                  {formatNumber(item.totalGrossRevenue)}
                </td>
                <td className="px-1 py-5 text-right text-rose-400">
                  {formatNumber(Math.abs(item.discount))}
                </td>
                <td className="px-1 py-5 text-right text-amber-500">
                  {formatNumber(Math.abs(item.returnsValue))}
                </td>
                <td className="px-1 py-5 text-right font-normal text-blue-900 bg-blue-50/40">
                  {formatNumber(item.netRevenue)}
                </td>
                <td className="px-1 py-5 text-right text-indigo-600">
                  {formatNumber(item.revenueOther)}
                </td>
                <td className="px-1 py-5 text-right text-rose-500">
                  {formatNumber(Math.abs(item.totalCogs))}
                </td>
                <td className="px-1 py-5 text-right font-normal text-emerald-700 bg-emerald-50/40">
                  {formatNumber(item.grossProfit)}
                </td>
                <td className="px-1 py-5 text-center bg-slate-50/20">
                  <button
                    onClick={() =>
                      onUpdate(
