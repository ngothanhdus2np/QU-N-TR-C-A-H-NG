    ...
    {staffRankings.length > 0 && (
      <div className="space-y-6">
        <div className="flex items-center gap-4 px-2">
          ...
          <div>
            <h3 className="text-xl font-semibold text-slate-900 uppercase tracking-tight">Bảng Vàng Doanh Số Tháng {selectedMonth.split('-').reverse().join('/')}</h3>
            <p className="text-2xs text-slate-400 font-normal uppercase tracking-widest mt-0.5">Xếp hạng hiệu quả đóng góp doanh thu của nhân sự</p>
          </div>
        </div>
            ...
            return (
              <div key={r.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-xl relative overflow-hidden group hover:-translate-y-1 transition-all">
                ...
                <div className="flex items-center justify-between mb-6 relative z-10">
                  ...
                  <span className={`text-2xs font-normal uppercase tracking-[0.2em] ${theme.text}`}>{theme.label}</span>
                </div>
                <div className="relative z-10">
                  <h4 className="text-xl font-semibold text-slate-900 uppercase tracking-tighter">{r.name}</h4>
                  <div className="mt-4 flex items-end gap-2">
                    <span className="text-2xl font-normal text-slate-800 tabular-nums">{(r.totalAmount || 0).toLocaleString()}</span>
                    <span className="text-2xs font-normal text-slate-400 mb-1.5">VNĐ</span>
                  </div>
                </div>
              </div>
            );
          ...
          <table className="w-full text-left border-collapse table-fixed">
            <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-30 backdrop-blur-sm bg-slate-50/90">
              <tr>
                ...
                {daysArray.map(day => (
                  <th key={day} className={`px-1 py-4 text-center text-2xs font-semibold w-[55px] border-r border-slate-100/50 ${isHoliday(day) ? 'bg-amber-50 text-amber-600' : 'text-slate-400'}`}>{day}</th>
                ))}
                <th className="px-3 py-4 text-center text-2xs font-semibold text-amber-600 uppercase bg-amber-50 sticky right-0 top-0 z-40 border-l border-slate-200 w-[100px] shadow-[-2px_0_5_rgba(0,0,0,0.02)]">Tổng</th>
              </tr>
            </thead>
