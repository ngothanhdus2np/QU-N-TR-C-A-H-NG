...
import { AppData, PayrollRecord, ViolationOccurrence, ViolationType, ResponsibilityApproval, SalaryPolicy } from '../../types';
import { calculateSeniority, determineCurrentPolicy } from '../../businessLogic';
import {
  BadgeCheck, Check, Info, PartyPopper, Printer, Save, UserMinus,
  ToggleLeft, ToggleRight, ShieldCheck, Loader2,
} from 'lucide-react';
...
const AccruedDetail = ({ label, value, isCut = false }: { label: string; value: number; isCut?: boolean }) => (
  <div className="flex justify-between items-center py-1 text-[10px] border-b border-slate-50/50">
    ...
    <div className="text-right">
      <span className={`font-black ${isCut ? 'text-rose-500' : 'text-indigo-600'}`}>{(value || 0).toLocaleString()}đ</span>
...
const SummaryTab: React.FC<Props> = ({
  draftPayrolls, archivedPayrolls, data, selectedMonth,
  violationOccurrences, violationTypes, responsibilityApprovals, policies,
  isProcessingSettlement, toggleResponsibilityApproval,
  handlePrintPayslip, handleFinalizeIndividual, handleSettlementAndResignation,
}) => (
  <div className="animate-in fade-in duration-500 space-y-16 pb-20">
    <section className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg"><BadgeCheck className="w-6 h-6" /></div>
          <div>
                ...
                <div className="space-y-1.5 pt-2">
                  ...
                  <div className="pl-4 space-y-1">
                    {currentPolicy.attendanceAllowance > 0 && (
                      <AccruedDetail label="Chuyên cần" value={Math.round((isAttendanceCut ? 0 : currentPolicy.attendanceAllowance) * proRateFactor)} isCut={isAttendanceCut} />
                    )}
                    {currentPolicy.cleaningAllowance > 0 && (
                      <AccruedDetail label="Vệ sinh" value={Math.round((isCleaningCut ? 0 : currentPolicy.cleaningAllowance) * proRateFactor)} isCut={isCleaningCut} />
                    )}
                    {currentPolicy.customerServiceAllowance > 0 && (
                      <AccruedDetail label="CSKH" value={Math.round((isCSKHCut ? 0 : currentPolicy.customerServiceAllowance) * proRateFactor)} isCut={isCSKHCut} />
                    )}
                    {currentPolicy.dinnerAllowance > 0 && (
                      <AccruedDetail label="Ăn tối" value={Math.round((isDinnerCut ? 0 : currentPolicy.dinnerAllowance) * workingDays)} isCut={isDinnerCut} />
                    )}
                    {currentPolicy.housingAllowance > 0 && (
                      <AccruedDetail label="Hỗ trợ ở" value={Math.round((isHousingCut ? 0 : currentPolicy.housingAllowance) * proRateFactor)} isCut={isHousingCut} />
                    )}
                    {currentPolicy.responsibilityAllowance > 0 && (
                      <div className="flex justify-between items-center py-1 group/resp">
                        <div className="flex items-center gap-2">
                          <button onClick={() => toggleResponsibilityApproval(payroll.employeeId)} className={`transition-all ${isApproved ? 'text-indigo-600' : 'text-slate-300'}`}>
              ...
              <div className="p-6 bg-slate-900 space-y-4">
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => handlePrintPayslip(payroll)} className="py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-white/10 bg-white/5 text-white hover:bg-white/10">
                    ...
                    <button
                      ...
                      className={`py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                        isArchived ? 'bg-slate-800/50 text-slate-500 cursor-not-allowed border border-slate-700' : 'bg-white text-slate-900 hover:bg-indigo-50'
                      }`}
                    >
