            ...
            {archivedPayrolls.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-20 text-center opacity-40">
                  ...
                  <p className="text-sm font-normal text-slate-500 uppercase">Chưa ghi nhận dữ liệu chốt lương cho tháng {selectedMonth}</p>
                </td>
              </tr>
            ) : (
              archivedPayrolls.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <p className="text-rose-600 font-normal">Trước: {(p.tetBonusBefore || 0).toLocaleString()}</p>
                    <p className="text-amber-600 font-normal">Sau: {(p.tetBonusAfter || 0).toLocaleString()}</p>
                  </td>
