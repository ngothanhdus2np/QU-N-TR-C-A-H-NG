          ...
          {misMetrics.benchmarks.map((benchmark, index) => (
            <div key={index} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex flex-col justify-between">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <span className="text-[10px] font-black text-slate-400 uppercase">{benchmark.name}</span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <h4 className="text-2xl font-black text-slate-900">{benchmark.actual.toFixed(1)}%</h4>
                    <span className="text-[10px] font-bold text-slate-400">/ {benchmark.target}% định mức</span>
                  </div>
                </div>
            ...
            {misMetrics.hotspots.length > 0 ? (
              misMetrics.hotspots.map((hotspot, index) => (
                <div key={index} className="p-6 bg-white/5 border border-white/10 rounded-[2.5rem] hover:bg-white/10 transition-all group">
                  ...
                  <div>
                    <h4 className="text-sm font-black text-slate-200 uppercase">{hotspot.category}</h4>
                    <p className="text-[10px] text-slate-400 mt-1 font-bold">Tăng {hotspot.diffPercent.toFixed(1)}% so với kỳ trước</p>
                  </div>
                </div>
              ))
            ) : (
          ...
          <div key={index} className="group p-6 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:bg-white hover:shadow-lg transition-all duration-300">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4 min-w-[250px]">
                ...
                <div>
                  <h4 className="text-sm font-black text-slate-800 uppercase leading-none">{category.name}</h4>
                  <p className="text-[10px] text-slate-400 font-bold mt-1">Tổng chi: {formatNumber(category.amount)}đ</p>
                </div>
              </div>
              ...
              <div className="flex-1 flex items-center gap-6">
                ...
                <div className="text-right min-w-[80px]">
                  <span className="text-base font-black text-slate-900">{category.ratio.toFixed(1)}%</span>
