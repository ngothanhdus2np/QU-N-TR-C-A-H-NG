...
type ExpenseSubTab = 'ledger' | 'recurring';
...
export const ExpenseTabs: React.FC<ExpenseTabsProps> = ({
  ...
  <div className="flex bg-slate-100 p-1.5 rounded-[2rem] w-fit mx-auto shadow-sm border border-slate-200">
    ...
    <button
      ...
      className={`flex items-center gap-3 px-8 py-4 rounded-[1.5rem] font-normal text-2xs uppercase tracking-widest transition-colors relative ${
    ...
    >
      ...
      {pendingRecurringCount > 0 && (
        <span className="absolute -top-1 -right-1 w-5 h-5 bg-rose-500 text-white text-2xs flex items-center justify-center rounded-full border-2 border-white animate-bounce">
          {pendingRecurringCount}
        </span>
      )}
    </button>
