  ...
  <div className="space-y-8 animate-in slide-in-from-bottom-4">
    <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
      <div className="lg:col-span-4 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 hover:border-blue-400 hover:bg-blue-50/20 transition-all p-10 shadow-sm relative group flex flex-col items-center justify-center text-center">
        <input type="file" accept=".csv, .xlsx, .xls" ref={fileInputRef} className="hidden" onChange={onFileUpload} />
        <button onClick={() => fileInputRef.current?.click()} className="w-20 h-20 rounded-full bg-slate-50 flex items-center justify-center border-2 border-dashed border-slate-200 group-hover:border-blue-300 group-hover:bg-white transition-all shadow-sm mb-6">
      ...
      <div className="lg:col-span-6 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl">
        ...
        <form onSubmit={onSubmitGroupRevenue} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <InputWrapper label="Ngày" icon={Calendar}><input type="date" required value={groupFormData.date} onChange={e => onSetGroupFormData({ ...groupFormData, date: e.target.value })} className="input-field py-2.5 font-normal text-[10px] border-none" /></InputWrapper>
            <InputWrapper label="SL bán" icon={Hash}><input type="number" required placeholder="0" value={groupFormData.quantity} onChange={e => onSetGroupFormData({ ...groupFormData, quantity: e.target.value })} className="input-field py-2.5 font-normal text-[10px] border-none" /></InputWrapper>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <InputWrapper label="Doanh thu" icon={DollarSign} color="text-blue-600"><input type="number" required placeholder="0" value={groupFormData.amount} onChange={e => onSetGroupFormData({ ...groupFormData, amount: e.target.value })} className="input-field py-2.5 font-normal text-[10px] border-none" /></InputWrapper>
            <InputWrapper label="Giá vốn" icon={ShoppingCart} color="text-slate-600"><input type="number" placeholder="0" value={groupFormData.cogs} onChange={e => onSetGroupFormData({ ...groupFormData, cogs: e.target.value })} className="input-field py-2.5 font-normal text-[10px] border-none" /></InputWrapper>
          </div>
          <InputWrapper label="Nhóm Hàng" icon={Layers}>
            <ProductGroupTreePicker
              groups={effectiveGroups}
              selectedPaths={(() => {
                const selectedGroup = effectiveGroups.find(group => group.id === groupFormData.groupId);
          ...
          <button type="submit" className="w-full bg-slate-900 hover:bg-black text-white py-3.5 rounded-xl font-normal text-[9px] uppercase tracking-widest transition-all shadow-xl">GHI NHẬN CHỈ SỐ</button>
        </form>
      </div>
    </div>
      ...
      <div className="px-8 py-6 bg-slate-50 border-b border-slate-100 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-white border border-slate-200 p-1.5 rounded-xl shadow-sm">
            <input type="date" value={filterStartDate} onChange={e => onSetFilterStartDate(e.target.value)} className="bg-transparent text-[10px] font-normal text-slate-600 outline-none" />
            <input type="date" value={filterEndDate} onChange={e => onSetFilterEndDate(e.target.value)} className="bg-transparent text-[10px] font-normal text-slate-600 outline-none ml-2" />
          </div>
        </div>
      </div>
                ...
                <tr key={`${row.month}_${row.fullPath}`} className="group hover:bg-slate-50 transition-all">
                  <td className="px-8 py-4" style={{ paddingLeft: `${row.level * 20}px` }}>
                    <div className="flex items-center gap-2">
                      {row.hasChildren && (
                        <button onClick={() => onToggleLedgerRow(row.month, row.fullPath)} className="p-1 rounded bg-slate-100 text-slate-500">
                          {isExp ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                        </button>
                      )}
                      <div>
                        {row.level === 1 && <p className="text-[8px] font-normal text-slate-400 mb-0.5">{String(row.month)}</p>}
                        <p className={`uppercase ${row.level === 1 ? 'text-slate-900' : 'text-slate-500 font-normal'}`}>{String(row.displayName)}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right font-normal text-emerald-700 bg-emerald-50/20">{formatNumber(row.profit)}</td>
                  <td className="px-4 py-4 text-center">
                    <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-normal text-[10px] ${marginColor}`}>
