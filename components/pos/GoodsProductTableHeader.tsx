  ...
  <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
    <tr>
      <th className="px-4 py-3 w-10">
        <input type="checkbox" className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" checked={isAllSelected} onChange={onToggleSelectAll} />
      </th>
      <th className="px-2 py-3 w-8"></th>
      {visibleColumns.includes('image') && <th className="px-2 py-3 w-20"></th>}
      <th className="px-4 py-3 text-left font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Mã hàng</th>
      ...
      {visibleColumns.includes('weight') && <th className="px-4 py-3 text-right font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Trọng lượng</th>}
      {visibleColumns.includes('allowPoints') && <th className="px-4 py-3 text-center font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Tích điểm</th>}
      {visibleColumns.includes('directSale') && <th className="px-4 py-3 text-center font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Bán TT</th>}
      {visibleColumns.includes('status') && <th className="px-4 py-3 text-center font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Trạng thái</th>}
      {visibleColumns.includes('warranty') && <th className="px-4 py-3 text-left font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Bảo hành</th>}
      {visibleColumns.includes('createdAt') && <th className="px-4 py-3 text-left font-black text-[10px] uppercase tracking-widest text-slate-500 whitespace-nowrap">Thời gian tạo</th>}
