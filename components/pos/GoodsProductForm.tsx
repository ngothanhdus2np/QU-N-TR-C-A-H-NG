    ...
    <div className="fixed inset-0 z-modal flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-50 w-full max-w-6xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                  <h3 className="font-semibold text-2xs uppercase text-slate-400 flex items-center gap-2 italic"><Package className="h-3 w-3" /> Định danh & Tài chính</h3>
                  <div>
                    <label className="text-2xs font-normal uppercase text-slate-500 mb-1 block">Tên sản phẩm *</label>
                  ...
                  <div>
                    <label className="text-2xs font-normal uppercase text-slate-500 mb-1 block">Giá bán lẻ</label>
              ...
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                  <h3 className="font-semibold text-2xs uppercase text-slate-400 flex items-center gap-2 italic"><History className="h-3 w-3" /> Phân loại & Quy cách</h3>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-2xs font-normal uppercase text-slate-500 mb-1 block">Nhóm hàng</label>
                    ...
                    <div><label className="text-2xs font-normal uppercase text-slate-500 mb-1 block">Thương hiệu</label><input className="w-full p-2 bg-slate-50 border rounded-lg outline-none" value={formData.brand} onChange={e => setFormData({ ...formData, brand: e.target.value })} placeholder="V.d: Nike" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div><label className="text-2xs font-normal uppercase text-slate-500 mb-1 block">Đơn vị tính</label><input className="w-full p-2 bg-slate-50 border rounded-lg outline-none" value={formData.unit} onChange={e => setFormData({ ...formData, unit: e.target.value })} /></div>
                    <div><label className="text-2xs font-normal uppercase text-slate-500 mb-1 block">Tồn hiện tại</label><input type="number" className="w-full p-2 bg-slate-50 border rounded-lg text-right font-normal outline-none" value={formData.stock} onChange={e => setFormData({ ...formData, stock: Number(e.target.value) })} /></div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-4">
                  <h3 className="font-semibold text-2xs uppercase text-slate-400 flex items-center gap-2 italic"><ImageIcon className="h-3 w-3" /> Thông tin khác</h3>
        ...
        <div className="p-4 bg-white border-t flex justify-end gap-3">
          <button onClick={onClose} className="px-6 py-2 border rounded-lg font-normal text-xs uppercase hover:bg-slate-50 transition-colors">
            Hủy bỏ
          </button>
          <button onClick={onSave} className="px-8 py-2 bg-indigo-600 text-white rounded-lg font-normal text-xs uppercase hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-colors">
            Lưu & Thêm vào phiếu
          </button>
        </div>
      </div>
