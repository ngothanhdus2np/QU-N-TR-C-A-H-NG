...
export const GoodsFilterSidebar: React.FC<GoodsFilterSidebarProps> = ({
  ...
  filterAttrs, setFilterAttrs,
  filterSupplier, setFilterSupplier,
  onClearAllFilters, onResetPage,
  uniqueCategories, categoryCounts,
  uniqueAttrTypes, uniqueLocations, uniqueBrands,
  lowStockCount,
}) => {
  // Category popup
  ...
  const hasActiveFilters = filterCategories.length > 0 || filterBrand || filterStock !== 'all' || filterLocation || filterAttrs.length > 0;
    ...
    <>
      ...
      {showCategoryPopup && (
        <div
          ...
          style={{ position: 'fixed', top: categoryPopupPos.top, left: categoryPopupPos.left, width: categoryPopupPos.width, zIndex: 9999 }}
        ...
        >
          ...
          <div className="overflow-y-auto max-h-64">
            {uniqueCategories
              ...
              .map(cat => (
                <label key={cat} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 cursor-pointer border-b border-slate-50">
                  <input
                    type="checkbox"
                    checked={pendingCategories.includes(cat)}
                    onChange={() => setPendingCategories(prev =>
                      prev.includes(cat) ? prev.filter(c => c !== cat) : [...prev, cat]
                    )}
                    className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4"
                  />
                  <span className="flex-1 text-sm text-slate-700">{cat}</span>
                  <span className="text-xs text-slate-400 tabular-nums">({categoryCounts[cat] || 0})</span>
                </label>
              ))}
          </div>
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50/60">
            <button
              onClick={() => setPendingCategories(pendingCategories.length === uniqueCategories.length ? [] : [...uniqueCategories])}
              className="text-sm text-indigo-600 font-semibold hover:underline"
            >
              {pendingCategories.length === uniqueCategories.length ? 'Bỏ chọn tất cả' : 'Chọn tất cả'}
            </button>
            <button
              onClick={() => { setFilterCategories(pendingCategories); onResetPage(); setShowCategoryPopup(false); }}
      ...
      {showAttrPopup && (
        <div
          ...
          style={{ position: 'fixed', top: attrPopupPos.top, left: attrPopupPos.left, width: attrPopupPos.width, zIndex: 9999 }}
          className="bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden"
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          ...
          <div className="overflow-y-auto max-h-64">
            {uniqueAttrTypes
              .filter(attr => attr.toLowerCase().includes(attrPopupSearch.toLowerCase()))
              .map(attr => (
                <label key={attr} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 cursor-pointer border-b border-slate-50">
                  <input
                    type="checkbox"
                    checked={pendingAttrs.includes(attr)}
                    onChange={() => setPendingAttrs(prev =>
                      prev.includes(attr) ? prev.filter(a => a !== attr) : [...prev, attr]
                    )}
                    className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 h-4 w-4"
                  />
                  <span className="flex-1 text-sm text-slate-700 uppercase font-semibold">{attr}</span>
                </label>
              ))}
          </div>
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50/60">
            <button
              onClick={() => setPendingAttrs(pendingAttrs.length === uniqueAttrTypes.length ? [] : [...uniqueAttrTypes])}
              className="text-sm text-indigo-600 font-semibold hover:underline"
            >
              {pendingAttrs.length === uniqueAttrTypes.length ? 'Bỏ chọn tất cả' : 'Chọn tất cả'}
            </button>
            <button
              onClick={() => { setFilterAttrs(pendingAttrs); onResetPage(); setShowAttrPopup(false); }}
              className="px-5 py-1.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-colors"
            >
              Áp dụng
            </button>
      ...
      {showStockPopup && (
        <div
          ...
          style={{ position: 'fixed', top: stockPopupPos.top, left: stockPopupPos.left, width: stockPopupPos.width, zIndex: 9999 }}
          className="bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden"
        >
          ...
          <div className="py-1">
            {([
              ...
              { v: 'low_stock', l: lowStockCount > 0 ? `Sắp hết hàng (${lowStockCount})` : 'Sắp hết hàng' },
            ] as const).map(opt => (
              <button
                key={opt.v}
                onClick={() => { setFilterStock(opt.v); onResetPage(); setShowStockPopup(false); }}
              ...
              >
                <span className={`h-4 w-4 rounded-full border-2 flex items-center justify-center shrink-0 ${filterStock === opt.v ? 'border-indigo-600 bg-indigo-600' : 'border-slate-300'}`}>
      ...
      {/* === Filter Aside === */}
      <aside className="w-64 shrink-0 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden">
        <div className="px-4 min-h-[52px] border-b border-slate-100 shrink-0 flex items-center justify-between">
          ...
          {hasActiveFilters && (
            <button onClick={onClearAllFilters} className="text-[10px] text-indigo-600 font-bold hover:underline">Xóa lọc</button>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            <div className="flex items-center justify-between mb-2">
              ...
              <button className="text-[10px] text-indigo-500 font-semibold hover:underline">Tạo mới</button>
            </div>
            <div
              ref={categoryTriggerRef}
              onClick={() => {
                if (showCategoryPopup) { setShowCategoryPopup(false); return; }
            ...
            >
              <span className={filterCategories.length > 0 ? 'text-slate-700 font-semibold' : ''}>
                {filterCategories.length === 0 ? 'Chọn nhóm hàng' : filterCategories.length === 1 ? filterCategories[0] : `${filterCategories.length} nhóm đã chọn`}
              </span>
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            ...
            <input
              ...
              onChange={e => { setFilterSupplier(e.target.value); onResetPage(); }}
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            ...
            <input
              ...
              value={filterBrand}
              onChange={e => { setFilterBrand(e.target.value); onResetPage(); }}
              list="brand-list"
              className="w-full px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg outline-none focus:border-indigo-400 transition-all placeholder:text-slate-300"
            />
            ...
            <div className="px-4 py-3 border-b border-slate-100">
              ...
              >
                <span className={filterAttrs.length > 0 ? 'text-slate-700 font-semibold' : ''}>
                  {filterAttrs.length === 0 ? 'Chọn thuộc tính' : filterAttrs.length === 1 ? filterAttrs[0] : `${filterAttrs.length} thuộc tính`}
                </span>
                <ChevronRight className="h-3.5 w-3.5 rotate-90 text-slate-400" />
              </div>
            </div>
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            ...
            <input
              ...
              onChange={e => { setFilterLocation(e.target.value); onResetPage(); }}
              list="location-list"
              className="w-full px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg outline-none focus:border-indigo-400 transition-all placeholder:text-slate-300"
            />
          ...
          <div className="px-4 py-3">
            ...
            <div
              ref={stockTriggerRef}
              onClick={() => {
                if (showStockPopup) { setShowStockPopup(false); return; }
            ...
            >
              <span className={filterStock !== 'all' ? 'text-slate-700 font-semibold' : 'text-slate-400'}>
                {filterStock === 'all' ? 'Tất cả' :
                 filterStock === 'in_stock' ? 'Còn hàng' :
                 filterStock === 'out_of_stock' ? 'Hết hàng' :
