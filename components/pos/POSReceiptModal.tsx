    ...
    <div className="bg-white rounded-[2.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-500 flex flex-col max-h-[90vh]">
      <div className="bg-slate-50 p-6 border-b border-slate-200 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-1">{order.orderCode} • {new Date(order.date).toLocaleString('vi-VN')}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 no-scrollbar print-area">
        ...
        <div className="mb-6 space-y-2">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-black text-slate-800">{order.customerName || 'Khách vãng lai'}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="font-bold text-slate-400 uppercase tracking-wider">Thu ngân:</span>
            <span className="font-black text-slate-800">{order.staffId}</span>
          </div>
        </div>
        ...
        <div className="space-y-3 border-t-2 border-dashed border-slate-200 pt-6">
          ...
          {order.discount > 0 && (
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-black text-rose-500">-{order.discount.toLocaleString()}đ</span>
            </div>
          )}
          <div className="flex justify-between items-center py-3 bg-slate-50 px-4 rounded-2xl">
            ...
            <span className="text-xl font-black text-indigo-600 italic leading-none">{order.finalAmount.toLocaleString()}đ</span>
          </div>
          <div className="flex justify-between items-center text-xs pt-2">
            <span className="font-bold text-slate-400 uppercase tracking-widest">Phương thức:</span>
            <span className="font-black text-slate-700 uppercase">{order.paymentMethod}</span>
          </div>
          <div className="flex justify-between items-center text-xs">
            <span className="font-bold text-slate-400 uppercase tracking-widest">Khách đưa:</span>
            <span className="font-black text-slate-700">{(cashReceived || order.finalAmount).toLocaleString()}đ</span>
          </div>
          {(cashReceived || order.finalAmount) > order.finalAmount && (
            <div className="flex justify-between items-center text-xs pt-1">
              ...
              <span className="font-black text-emerald-600">{((cashReceived || order.finalAmount) - order.finalAmount).toLocaleString()}đ</span>
            </div>
          )}
        </div>
