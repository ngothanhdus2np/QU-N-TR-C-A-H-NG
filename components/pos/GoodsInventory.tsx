...
const printProductLabels = (selectedProducts: POSProduct[], labelsPerProduct: number) => {
  ...
  const labelHtml = labels
    .map(product => {
      ...
      return `<section class="label">${showName ? `<div class="name">${escapeLabelText(product.name)}</div>` : ''}${buildCode128Svg(code, { height: 50, ariaLabel: false })}${showCode ? `<div class="code">${escapeLabelText(code)}</div>` : ''}${showPrice ? `<div class="price">${escapeLabelText(product.salePrice.toLocaleString('vi-VN'))} VNĐ</div>` : ''}</section>`;
    })
    .join('');
