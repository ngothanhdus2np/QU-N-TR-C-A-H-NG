    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-rose-50 to-rose-100 rounded-xl p-4 border border-rose-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-normal text-rose-700 uppercase tracking-wide">Nợ hiện tại</span>
          </div>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-xl p-4 border border-indigo-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-normal text-indigo-700 uppercase tracking-wide">Tổng mua</span>
          </div>
          <div className="text-2xl font-normal text-indigo-700">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Mã NCC</label>
              <p className="text-sm font-normal text-slate-800 mt-1">
                {supplier.code || supplier.id.slice(0, 8)}
              </p>
            </div>
            <div>
              <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Tên NCC</label>
              <p className="text-sm font-normal text-slate-800 mt-1">{supplier.name}</p>
            </div>
          </div>

          {supplier.group && (
            <div>
              <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Nhóm</label>
              <p className="text-sm font-normal text-slate-800 mt-1">{supplier.group}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            {supplier.phone && (
              <div>
                <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Điện thoại</label>
                <p className="text-sm font-normal text-slate-800 mt-1">{supplier.phone}</p>
              </div>
            )}
            {supplier.email && (
              <div>
                <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Email</label>
                <p className="text-sm font-normal text-slate-800 mt-1">{supplier.email}</p>
              </div>
            )}
          </div>
          ...
          {supplier.address && (
            <div>
              <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Địa chỉ</label>
              <p className="text-sm font-normal text-slate-800 mt-1">{supplier.address}</p>
            </div>
          )}

          <div>
            <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Trạng thái</label>
            <div className="mt-1">
              <span
                className={`inline-block px-2 py-1 rounded-full text-[10px] font-normal uppercase ${
          ...
          {supplier.notes && (
            <div>
              <label className="text-xs font-normal text-slate-500 uppercase tracking-wide">Ghi chú</label>
    ...
    <div className="p-6">
      {debtsWithBalance.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-sm font-normal text-slate-400">Chưa có giao dịch công nợ</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                {debtsWithBalance.map(debt => (
                  <tr key={debt.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-normal uppercase ${
                          debt.type === 'purchase'
                    ...
                    <td className="px-4 py-3 text-right font-normal text-slate-700">
    ...
    <div className="p-6">
      {purchases.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-sm font-normal text-slate-400">Chưa có phiếu nhập hàng</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="overflow-x-auto">
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
      <div className="bg-slate-50 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-white border-b border-slate-200 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-lg font-normal text-slate-900">{supplier.name}</h2>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onEdit}
              className="flex items-center gap-2 px-3 py-2 bg-indigo-600 text-white rounded-lg text-xs font-normal hover:bg-indigo-700 transition-colors"
            >
            ...
            <button
              onClick={onDelete}
              className="flex items-center gap-2 px-3 py-2 bg-rose-600 text-white rounded-lg text-xs font-normal hover:bg-rose-700 transition-colors"
            >
