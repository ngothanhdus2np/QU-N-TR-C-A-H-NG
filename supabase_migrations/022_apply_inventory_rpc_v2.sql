-- =============================================================================
-- 022 — Fix INV-RPC-01 (audit 2026-07-02): áp RPC tồn kho _v2 lên DB thật
-- =============================================================================
-- Migration 013 định nghĩa apply/delete_inventory_transaction_with_stock_v2
-- (atomic, ĐỦ nhánh Sale/Return/Import/PurchaseReturn/Check, xử lý allow_negative)
-- nhưng CHƯA từng được áp lên DB (baseline tưởng đã vá tay — audit phát hiện sai).
-- Hệ quả: route /api/data/inventory/apply gọi _v2 → lỗi 42883 → rơi vào fallback
-- nhiều-bước không atomic. Bản legacy apply_inventory_transaction_with_stock trên
-- DB còn THIẾU nhánh Sale/Return (chỉ Import+Check) → bom nổ chậm.
--
-- Migration này áp lại đúng nội dung 013 (CREATE OR REPLACE idempotent) để mọi
-- đơn dùng RPC atomic 1-transaction. Khi _v2 tồn tại, route KHÔNG còn rơi vào
-- legacy cho Sale/Return (legacy chỉ còn phục vụ type 'Check' vốn đã đúng), nên
-- landmine "legacy thiếu nhánh Sale/Return" được vô hiệu hoá.
-- =============================================================================
