-- Fix: delete_pos_order_tx không hoàn tồn kho khi xóa đơn hàng.
-- Vòng lặp hoàn tồn kho dùng "status <> 'cancelled'" — nhưng inventory_transactions
-- loại Sale có status mặc định NULL, và NULL <> 'cancelled' cho kết quả NULL (không
-- phải TRUE) theo logic 3 giá trị của SQL, nên dòng NULL bị loại khỏi vòng lặp:
-- tồn kho không bao giờ được cộng lại, giao dịch cũng không được đánh dấu cancelled.
-- cancel_pos_return_tx/edit_pos_order_tx đã dùng đúng COALESCE(status, '') <> 'cancelled'
-- — migration này chỉ áp dụng cùng cách xử lý cho delete_pos_order_tx.
...
BEGIN
  ...
  FOR tx IN SELECT * FROM inventory_transactions WHERE reference_id = p_order_id::TEXT AND COALESCE(status, '') <> 'cancelled'
  LOOP
