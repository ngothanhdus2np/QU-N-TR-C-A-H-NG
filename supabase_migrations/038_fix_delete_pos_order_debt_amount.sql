-- Fix: delete_pos_order_tx tham chiếu cột pos_customers.debt_amount không tồn tại
-- (cột này chưa từng thực sự được tạo trên DB dù supabase_setup.sql có ghi ALTER TABLE
-- ADD COLUMN — xem TODO.md DEBT-AMOUNT-COL-0818). Nguồn sự thật duy nhất cho công nợ
-- khách hàng là bảng customer_debt_history (đã chốt từ fix 2026-08-14), place_pos_order_tx/
-- edit_pos_order_tx/cancel_pos_return_tx đều không đụng debt_amount. Migration này chỉ bỏ
-- dòng UPDATE debt_amount khỏi delete_pos_order_tx cho khớp — không thêm lại cột.
